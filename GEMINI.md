...
10. **SceneCore parent detach cascades to children before Compose `DisposableEffect.onDispose` runs.** When the activity tears down, detaching a parent `Entity` (via `parent = null`) lets GC reclaim its child `GltfModelEntity`s, so each child's `onDispose { ent.removeComponent(ic); ent.parent = null }` then throws `Entity.DisposedException` and aborts `Activity.performDestroy()` — the next process bootstraps with empty state and looks like the user's work vanished. Wrap teardown in `if (!ent.isDisposed) runCatching { ... }`. Same applies to setPose racing with Filament's material binding on freshly-created entities: re-issuing `setScale(Vector3, Space.PARENT)` on every recomposition (vs once at create with the scalar overload) aborts the render thread with `split_engine_bridge: unknown material instance id`.
    ...
    **Alpha15 deprecations applied (2026-05-20):**
    ...
    - **`CustomMesh` COLOR attribute requires `UBYTE4_NORM` (RGBA, normalized 0.0 - 1.0).** Using `FLOAT3` or `FLOAT4` compiles fine but throws `IllegalArgumentException: Incompatible type FLOAT4 for attribute COLOR.` at runtime on the device. Always use `VertexAttributeType.UBYTE4_NORM` for `COLOR`, allocate/store the vertex color data as a `ByteArray` (4 bytes per vertex: R, G, B, A scaled to 0..255), and upload it using `ByteBuffer.allocateDirect` / `colBuf.put(colors).flip()`.
    - **`CustomMesh`, materials, and direct `ByteBuffer`s must be strongly referenced in Kotlin for as long as their rendering `MeshEntity` is active.** The SceneCore wrapper does not keep strong Java references from `MeshEntity` to the source `CustomMesh`, `KhronosUnlitMaterial`, or direct `ByteBuffer`s. If they are created as local variables inside a `LaunchedEffect`, they become eligible for GC once the block finishes or is re-keyed. Reclaiming a direct `ByteBuffer` deallocates its underlying native memory, and GC'ing the `CustomMesh` triggers native destruction via finalizer (`nDestroyCustomMesh` in `/arm64/libimpress_api_jni.so`), resulting in a native segfault or abort inside Filament. Keep strong references in Compose `remember` state (e.g. via `BboxResources`).
    - **Cancellation Race and Parallel GC Finalization Safeguards:**
      1. **Coroutine Cancellation Safety:** If a `LaunchedEffect` coroutine is cancelled (e.g., during model translation/rotation changes) while suspended at `awaitFrame()`, it aborts immediately. The local variables are GC'd, but the newly created `MeshEntity` remains attached to the native scene graph and crashes when Filament renders it. To solve this, wrap the `LaunchedEffect` body in a `try-finally` block. In `finally`, if `activeResources` has not been successfully set to the new resources, clean up the orphaned `newRes` via `disposeResourcesDeferred` in the background `bboxCleanupScope`.
      2. **Parallel GC Finalization Order:** When resources are cleared, the `MeshEntity`, `CustomMesh`, and `KhronosUnlitMaterial` JVM objects become eligible for GC simultaneously. If the JVM finalizer thread finalizes `CustomMesh` or `material` *before* `MeshEntity` is finalized and its native Filament node is destroyed, the native engine aborts due to outstanding borrows or missing material IDs. Therefore, `disposeResourcesDeferred(resources)` must: (a) wait 2 frames (`awaitFrame()`) to allow the render queue to settle; (b) detach the entity (`parent = null`); (c) extract the assets (`mesh`, `material`, `posBuf`, `colBuf`, `idxBuf`) into local variables; and (d) delay for a generous window (`delay(5000)`) holding onto the extracted assets, performing a dummy hash read to guarantee the compiler doesn't optimize them away, ensuring the native node is destroyed *before* the mesh/material memory is freed.
