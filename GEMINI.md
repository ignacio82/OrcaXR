...
| 3D | `androidx.xr.scenecore:1.0.0-alpha15` for interaction proxies; `SpatialGltfModel` for visible generated GLBs including the build plate, previews, toolpaths, and selection outline |
...
1. **`SpatialPanel` / 3D content must launch in Full Space, not just request it later.** A `Subspace { SpatialPanel { Compose UI } }` placed inside an Activity that is still in *Home Space* mode produces a panel frame with system chrome but a **solid black content surface**; `GltfModelEntity` children can also log as attached while the user sees no build plate/model because 3D content is culled outside Full Space. The manifest must declare `<property android:name="android.window.PROPERTY_XR_ACTIVITY_START_MODE" android:value="XR_ACTIVITY_START_MODE_FULL_SPACE_MANAGED" />` under `<application>` and still call `session.scene.requestFullSpaceMode()` from a `LaunchedEffect(session)`. **Do not gate `SpatialGltfModel` composition on `SpatialCapability.SPATIAL_3D_CONTENT`:** on a live Galaxy XR relaunch on 2026-05-24, `dumpsys` showed `xrDesktopMode=full-space-managed` while SceneCore's capability snapshot remained `3dContent=false`, and the gate suppressed an otherwise renderable plate/model/selection outline. Retain capability logging for diagnostics only. ADB/dumpsys symptom of the original Home Space failure: the task shows `xrDesktopMode=home-space` after launch.
   - **Standby Launch Space Trap:** When deploying via `android run` / `am start`, the headset is typically asleep on the desk. The OpenXR session starts, but the OS ignores the Full Space request because there is no active wearer. Simply overriding `onResume` to request Full Space is not enough if the wearer wakes the headset later. To break out of Home Space culling, we intercept the spatial visibility transition `WITHIN_FIELD_OF_VIEW` in our visibility listener (which fires precisely when the user puts the headset on and looks at the app) to forcefully call `runCatching { session.scene.requestFullSpaceMode() }` and update the workspace placement pose using the live, non-zero `recommendedContentBoxInFullSpace.center`.
...
2. **Modal plate-grab via a single `MovableComponent` on the workspace, gated by a `plateMovable` toggle in `PlateTabPanel`.** The earlier orbiter-cube design (two 5 cm cubes outside the bed corners) was replaced because (a) the cubes never reliably caught hand-pinches on Galaxy XR, (b) re-keying the orbiter `LaunchedEffect` on `bedW`/`bedH` synchronously disposed in-flight `GltfModelEntity`s when the saved profile loaded after defaults, tripping gotcha #13, and (c) two visible affordances next to each model gizmo invited mis-grabs. Current design:
   ...
   - **Render anchor ownership:** `OrcaXR-workspace-placement` is a direct child of `session.scene.activitySpace` and must be assigned to `session.scene.keyEntity`; `OrcaXR-workspace` is its child carrying `(0, WORKSPACE_Y_OFFSET, 0)` and `WORKSPACE_ROTATION`. SceneCore applies its recommended Full Space/recenter pose and scale to the key entity (the default key is the main panel), so an independent GLB workspace can attach successfully but remain outside the focal placement. Keep the printer-axis rotation on the child, not the key entity. Do not copy the panel shell's `-1.5 m` Z offset onto the workspace: a live Galaxy XR run placed the build plate and dragon at real-world `z=-3.23 m`.
...
15. **Use `SpatialGltfModel` for the visible generated-GLB path while retaining entity-backed interaction proxies.** On Galaxy XR, `GltfModelEntity` can load, attach, report a real-world pose within field of view, and still produce no visible plate/model/toolpath. The official XR sample renders its model through `SpatialGltfModel`; OrcaXR now mirrors that rendering route with `ByteArraySpatialGltfModelSource` for cache-generated GLBs. Keep the existing `GltfModelEntity` while paint/selection/grab require attaching `InteractableComponent`/`MovableComponent`; `SpatialGltfModel` exposes nodes/material overrides but not an entity component hook. The debug-only probe (`SpatialGltfProbeActivity` + `SpatialGltfProbeTest`) remains the lifecycle stress test.
