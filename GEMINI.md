...
20. **FullSpectrum mixed-filament integration: data-model + wire format are at FS v0.9.9 parity, engine emission is NOT ported.** As of 2026-05-10 the patches 0015-0024 land all data structures (`MixedFilament.{cpp,hpp}`, `filament_mixer*`, `LocalZOrderOptimizer.hpp`), register the 17 PrintConfig keys (`mixed_filament_*`, `dithering_*`, `local_z_wipe_tower_purge_lines`), and add the `MixedFilamentManager` member on `Print`. The Kotlin side (`MixedFilamentEntry`, `MixedFilamentStore.toMixedFilamentDefinitions`, `parseMixedDefinitionsForKotlin` in MainActivity, `OrcaProfileLoader.SAFE_KEYS`, `SlicerEngine.PROJECT_OVERRIDE_KEYS`, `MixedAdvancedEditor` UX, MCP `set_mixed_filament_row` / `delete_mixed_filament_row` / `list_mixed_filaments`) all speak the v0.9.9 wire format and round-trip cleanly. **Patches 0027-0044 landed + verified compile-clean 2026-05-11.** Every FS file delta NOT entangled with the Local-Z planner has been ported:
  ...
  - 0044 `Print.hpp` Local-Z type scaffolding: `LocalZInterval` struct + `SubLayerPlan` struct + `PrintObject::local_z_intervals()` / `local_z_sublayer_plan()` / `set_local_z_plan()` / `clear_local_z_plan()` accessors + `m_local_z_intervals` / `m_local_z_sublayer_plan` private members + `WipeTowerData::local_z_tool_changes` field. **Dead code** — nothing populates or reads these yet. Foundation for the future Local-Z planner port (estimated ~3,700 LoC across `PrintObjectSlice.cpp` planner + `WipeTower2` purge sequencing + `Print.cpp` consumption + `GCode.cpp` Z-walk emission). With patch 0044 landed, the type surface is stable so future planner work has nothing more to add to Print.hpp.
