...
20. **FullSpectrum mixed-filament integration: data-model + wire format are at FS v0.9.9 parity, engine emission is NOT ported.** As of 2026-05-10 the patches 0015-0024 land all data structures (`MixedFilament.{cpp,hpp}`, `filament_mixer*`, `LocalZOrderOptimizer.hpp`), register the 17 PrintConfig keys (`mixed_filament_*`, `dithering_*`, `local_z_wipe_tower_purge_lines`), and add the `MixedFilamentManager` member on `Print`. The Kotlin side (`MixedFilamentEntry`, `MixedFilamentStore.toMixedFilamentDefinitions`, `parseMixedDefinitionsForKotlin` in MainActivity, `OrcaProfileLoader.SAFE_KEYS`, `SlicerEngine.PROJECT_OVERRIDE_KEYS`, `MixedAdvancedEditor` UX, MCP `set_mixed_filament_row` / `delete_mixed_filament_row` / `list_mixed_filaments`) all speak the v0.9.9 wire format and round-trip cleanly. **Patches 0027-0066 landed + verified compile-clean 2026-05-11. Local-Z gcode emission is live end-to-end.** Every FS file delta NOT entangled with the Local-Z planner has been ported:
  ...
  - 0066 `GCode.cpp` **Local-Z phase-b Z-walk emission (LZ5g-emit, 866 LoC)**: adds the actual gcode-emitting branch. When `local_z_pass_refs` is non-empty: emits `; local-z phase-b perimeter passes begin\n`, defines `emit_local_z_toolchange` (calls `m_wipe_tower->tool_change(.../*local_z_unplanned=*/true)` to consume preplanned Local-Z tcrs from patches 0058/0060, or falls back to `set_extruder`) + `emit_local_z_pass_for_extruder` (retract+lazy_lift→`travel_to_z(pass_z)`→`sort_print_object_instances`→per-island `extrude_perimeters` twice for non-infill-first/infill-first), runs `emit_local_z_legacy` legacy scheduler (print_z-grouped chunks via `LocalZOrderOptimizer::order_bucket_extruders`), emits `; local-z phase-b perimeter passes end\n`. Dependency-chain mode (FS 5826-6000, ~170 LoC topological scheduler) falls through to legacy. Adds top-of-file `#include "LocalZOrderOptimizer.hpp"`. OrcaXR adapts `m_writer.extruder()`→`m_writer.filament()`. **With patch 0066, Local-Z mode actually produces sub-Z gcode at print time when `dithering_local_z_mode` is on.** **What remains:** LZ5h — base-mask exclude (~50 LoC) to subtract painted geometry from the normal flow so it isn't double-printed (right now: over-extrusion at painted regions, not missing geometry — safe for first hardware validation); plus Phase 6 hardware print on Snapmaker U1.
