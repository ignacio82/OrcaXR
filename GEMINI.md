...
20. **FullSpectrum mixed-filament integration: data-model + wire format are at FS v0.9.9 parity, engine emission is NOT ported.** As of 2026-05-10 the patches 0015-0024 land all data structures (`MixedFilament.{cpp,hpp}`, `filament_mixer*`, `LocalZOrderOptimizer.hpp`), register the 17 PrintConfig keys (`mixed_filament_*`, `dithering_*`, `local_z_wipe_tower_purge_lines`), and add the `MixedFilamentManager` member on `Print`. The Kotlin side (`MixedFilamentEntry`, `MixedFilamentStore.toMixedFilamentDefinitions`, `parseMixedDefinitionsForKotlin` in MainActivity, `OrcaProfileLoader.SAFE_KEYS`, `SlicerEngine.PROJECT_OVERRIDE_KEYS`, `MixedAdvancedEditor` UX, MCP `set_mixed_filament_row` / `delete_mixed_filament_row` / `list_mixed_filaments`) all speak the v0.9.9 wire format and round-trip cleanly. **Patches 0027-0062 landed + verified compile-clean 2026-05-11.** Every FS file delta NOT entangled with the Local-Z planner has been ported:
  ...
  - 0062 `GCode.cpp` `process_layer` Local-Z pass-ref prep (LZ5g-prep, 352 LoC): `local_z_perimeter_phase_b_enabled` flag (from contexts) + `LocalZPassRef` struct + `local_z_pass_refs` vector sorted by (plan->print_z, layer_to_print_idx, plan->pass_index) + three lambdas (`same_local_z_pass_group`, `local_z_bucket_extruders`, `shared_local_z_extruder`). Diagnostic log on prep. Always-empty until `LocalZPassBucket::by_extruder` populates. **What remains:** LZ5g-emit — (a) populate `LocalZPassBucket::by_extruder` during per-extruder grouping (FS ~5258), (b) the emit branch (FS lines 5665-6036, ~370 LoC) that walks pass_refs, calls `tool_change(.../*local_z_unplanned=*/true, sub_z)` for each Local-Z toolchange, and emits perimeters at the sub-Z print_z via `extrude_perimeters` after travel_to_z (legacy print_z-group mode + dependency-chain topological scheduler — same two modes as patch 0060's `collect_local_z_wipe_tower_toolchanges`).
