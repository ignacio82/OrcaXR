...
20. **FullSpectrum mixed-filament integration: data-model + wire format are at FS v0.9.9 parity, engine emission is NOT ported.** As of 2026-05-10 the patches 0015-0024 land all data structures (`MixedFilament.{cpp,hpp}`, `filament_mixer*`, `LocalZOrderOptimizer.hpp`), register the 17 PrintConfig keys (`mixed_filament_*`, `dithering_*`, `local_z_wipe_tower_purge_lines`), and add the `MixedFilamentManager` member on `Print`. The Kotlin side (`MixedFilamentEntry`, `MixedFilamentStore.toMixedFilamentDefinitions`, `parseMixedDefinitionsForKotlin` in MainActivity, `OrcaProfileLoader.SAFE_KEYS`, `SlicerEngine.PROJECT_OVERRIDE_KEYS`, `MixedAdvancedEditor` UX, MCP `set_mixed_filament_row` / `delete_mixed_filament_row` / `list_mixed_filaments`) all speak the v0.9.9 wire format and round-trip cleanly. **Patches 0027-0059 landed + verified compile-clean 2026-05-11.** Every FS file delta NOT entangled with the Local-Z planner has been ported:
  ...
  - 0059 `Print.cpp/WipeTower2.hpp` Local-Z reserve-box hook (LZ5d, 203 LoC): captures `wipe_tower.get_local_z_reserve_boxes()` into `m_wipe_tower_data.local_z_reserve_boxes` after the legacy `WipeTower2::generate(...)` call so the LZ5b plumbing has live data; promotes `local_z_tool_change` / `get_local_z_reserve_boxes` from the private section to public in `WipeTower2.hpp` (they were initially landed next to private LZ4b helpers; Print.cpp call sites need public visibility). The "new" `WipeTower` variant branch doesn't expose Local-Z (only WipeTower2 does) — its reserve-box assignment is intentionally skipped with default-empty defaults. **What remains for Local-Z end-to-end at print time:** (1) LZ5e — port `collect_local_z_wipe_tower_toolchanges` (FS ~280 LoC) + `plan_local_z_toolchange`/`plan_local_z_reserve` hookups in the wipe-tower planning loop so the reserve_boxes actually populate; (2) LZ5f — GCode.cpp call sites that invoke `WipeTowerIntegration::tool_change(extruder, finish_layer, /*local_z_unplanned=*/true, sub_z)` between sub-Z extrusions (the actual Z-walk emit loop that scans `PrintObject::local_z_sublayer_plan()` records — FS GCode.cpp lines 4958-5800+).
