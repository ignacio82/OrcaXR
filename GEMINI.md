...
20. **FullSpectrum mixed-filament integration: data-model + wire format are at FS v0.9.9 parity, engine emission is NOT ported.** As of 2026-05-10 the patches 0015-0024 land all data structures (`MixedFilament.{cpp,hpp}`, `filament_mixer*`, `LocalZOrderOptimizer.hpp`), register the 17 PrintConfig keys (`mixed_filament_*`, `dithering_*`, `local_z_wipe_tower_purge_lines`), and add the `MixedFilamentManager` member on `Print`. The Kotlin side (`MixedFilamentEntry`, `MixedFilamentStore.toMixedFilamentDefinitions`, `parseMixedDefinitionsForKotlin` in MainActivity, `OrcaProfileLoader.SAFE_KEYS`, `SlicerEngine.PROJECT_OVERRIDE_KEYS`, `MixedAdvancedEditor` UX, MCP `set_mixed_filament_row` / `delete_mixed_filament_row` / `list_mixed_filaments`) all speak the v0.9.9 wire format and round-trip cleanly. **Patches 0027-0065 landed + verified compile-clean 2026-05-11.** Every FS file delta NOT entangled with the Local-Z planner has been ported:
  ...
  - 0065 `GCode.cpp` Local-Z perimeter clipping into pass_buckets (LZ5g population step 3, 675 LoC): adds the per-extrusion clipping branch inside `process_layer`'s per-region perimeter loop. When entity_type is PERIMETERS + the current LayerToPrint has a non-empty `mixed_masks_union`, clips each pass_bucket's compensated masks out of the extrusion via `clip_extrusion_collection_for_local_z`, validates clipped path heights match `plan->flow_height`, stashes the clipped EEC in a function-scope `local_z_clipped_collections` unique_ptr vector, and routes it into `pass_bucket.by_extruder` via `object_islands_by_extruder`. **ADDITIVE** — original extrusions still flow through the normal by_extruder path; the future emit-branch patch will subtract painted geometry from the base path. With patch 0065 landed, `local_z_pass_refs` is non-empty for Local-Z layers and the "phase-b prepared" log shows `perimeter_passes > 0`. **What remains:** LZ5g-emit — the actual emit branch (FS 5665-6036, ~370 LoC) that walks `local_z_pass_refs`, calls `tool_change(.../*local_z_unplanned=*/true, sub_z)` per Local-Z toolchange, and emits perimeters at the sub-Z print_z via `travel_to_z` + `extrude_perimeters`; plus the base-mask exclude path so painted geometry isn't double-printed by the normal flow.
