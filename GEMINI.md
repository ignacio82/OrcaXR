...
21. **Most multi-object 3MFs are decomposed into separate STLs on import, but Bambu / Orca assemblies stay as one container.** To allow per-object selection and transform (Phase B9), generic multi-object 3MFs extract each `ModelObject::mesh()` to a temporary STL in `cacheDir/extracted/<hash>/`. Each `PlacedModel` points to one of these STLs, sharing a `groupId` (the source 3MF's hash). STL has no material metadata, so carry the source object's one-based default extruder in `PlacedModel.previewFilamentIndex`; palette changes must re-bake these extracted STL previews. Bambu / Orca assembly 3MFs with `Metadata/model_settings.config` `<assemble_item>` entries often encode painted multi-color models as multiple assembled objects; splitting them loses `paint_color` metadata and makes every part fall back to a single default material. Keep those archives as a single `PlacedModel` and preserve their embedded relative layout in native preview / slicing. `nativeSliceMulti` slices decomposed STL sets.
