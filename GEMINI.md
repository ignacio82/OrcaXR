...
20. **FullSpectrum mixed-filament integration: data-model + wire format are at FS v0.9.9 parity, engine emission is NOT ported.** As of 2026-05-10 the patches 0015-0024 land all data structures (`MixedFilament.{cpp,hpp}`, `filament_mixer*`, `LocalZOrderOptimizer.hpp`), register the 17 PrintConfig keys (`mixed_filament_*`, `dithering_*`, `local_z_wipe_tower_purge_lines`), and add the `MixedFilamentManager` member on `Print`. The Kotlin side (`MixedFilamentEntry`, `MixedFilamentStore.toMixedFilamentDefinitions`, `parseMixedDefinitionsForKotlin` in MainActivity, `OrcaProfileLoader.SAFE_KEYS`, `SlicerEngine.PROJECT_OVERRIDE_KEYS`, `MixedAdvancedEditor` UX, MCP `set_mixed_filament_row` / `delete_mixed_filament_row` / `list_mixed_filaments`) all speak the v0.9.9 wire format and round-trip cleanly. **Patches 0027-0061 landed + verified compile-clean 2026-05-11.** Every FS file delta NOT entangled with the Local-Z planner has been ported:
  ...
  - 0061 `GCode.cpp` `process_layer` Local-Z context setup (LZ5f scaffold, 255 LoC): adds `LocalZPassBucket` / `LocalZLayerContext` struct definitions + the per-LayerToPrint setup loop that walks `PrintObject::local_z_sublayer_plan()` and accumulates one bucket per `split_interval` plan with perimeter-expanded per-extruder masks (compensation = 0.10 mm). Gated off when `is_anything_overridden` is true (wiping-overrides not yet Local-Z aware). All data structures populated then discarded — **dead code until LZ5g consumes pass_buckets**. **What remains:** LZ5g — the actual emit loop (~630 LoC of FS GCode.cpp 5170-5800 restructure that routes per-region extrusions into the right pass_bucket, emits sub-Z Z-walks, and calls `WipeTowerIntegration::tool_change(.../*local_z_unplanned=*/true, sub_z)` between sub-Z extrusion blocks).
