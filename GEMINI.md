...
| 3D | `androidx.xr.scenecore:1.0.0-alpha15` for interaction proxies; `SpatialGltfModel` for visible generated GLBs including the build plate, previews, toolpaths, and selection outline, all parented beneath one Compose-bridged workspace `SceneCoreEntity` |
...
Mobile support / print setup state is single-model and split deliberately: `MobileShell` owns the live phone slicer buffers (`slicerPaintIndex`, `slicerSupportFlags`, transform, per-object overrides, adaptive layer profile/ranges) that feed `SlicerScreen.runSlice`, while `MobileWorkspaceBinding` publishes the same state as a synthetic `PlacedModel(id="mobile_loaded")` so MCP tools can read/mutate it. Any path that loads a different phone model must clear both sides' stale per-model state: output path, color paint, support paint, object overrides, layer profiles/ranges, and transform in `MobileShell`, plus `stateByModel` / BVH cache in `MobileWorkspaceBinding`. Otherwise the next phone slice can inherit support facets or object settings from the previous file even though the UI appears to show a fresh model.

## Jetpack XR rendering gotchas

1. **`SpatialPanel` / 3D content must launch in Full Space, not just request it later.** A `Subspace { SpatialPanel { Compose UI } }` placed inside an Activity that is still in *Home Space* mode produces a panel frame with system chrome but a **solid black content surface**; `GltfModelEntity` children can also log as attached while the user sees no build plate/model because 3D content is culled outside Full Space. The manifest must declare `<property android:name="android.window.PROPERTY_XR_ACTIVITY_START_MODE" android:value="XR_ACTIVITY_START_MODE_FULL_SPACE_MANAGED" />` under `<application>` and still call `session.scene.requestFullSpaceMode()` from a `LaunchedEffect(session)`. **Do not gate `SpatialGltfModel` composition on `SpatialCapability.SPATIAL_3D_CONTENT`:** on a live Galaxy XR relaunch on 2026-05-24, `dumpsys` showed `xrDesktopMode=full-space-managed` while SceneCore's capability snapshot remained `3dContent=false`, and the gate suppressed an otherwise renderable plate/model/selection outline. Retain capability logging for diagnostics only. ADB/dumpsys symptom of the original Home Space failure: the task shows `xrDesktopMode=home-space` after launch.
   - **Standby Launch Space Trap:** When deploying via `android run` / `am start`, the headset is typically asleep on the desk. The OpenXR session starts, but the OS ignores the Full Space request because there is no active wearer. Simply overriding `onResume` to request Full Space is not enough if the wearer wakes the headset later. To break out of Home Space culling, intercept the spatial visibility transition `WITHIN_FIELD_OF_VIEW` in the visibility listener (which fires precisely when the user puts the headset on and looks at the app) and forcefully call `runCatching { session.scene.requestFullSpaceMode() }`. The Compose `Subspace` then applies its recommended placement to the bridged workspace.
...
2. **Modal plate-grab via a single `MovableComponent` on the workspace, gated by a `plateMovable` toggle in `PlateTabPanel`.** The earlier orbiter-cube design (two 5 cm cubes outside the bed corners) was replaced because (a) the cubes never reliably caught hand-pinches on Galaxy XR, (b) re-keying the orbiter `LaunchedEffect` on `bedW`/`bedH` synchronously disposed in-flight `GltfModelEntity`s when the saved profile loaded after defaults, tripping gotcha #13, and (c) two visible affordances next to each model gizmo invited mis-grabs. Current design:
   ...
   - **Render/input anchor ownership:** `OrcaXR-workspace` must be supplied as a `SceneCoreEntity` factory inside the same `Subspace` as every visible `SpatialGltfModel`, with `(0, WORKSPACE_Y_OFFSET, 0)` and `WORKSPACE_ROTATION` on that bridged workspace modifier. **Every workspace `SpatialGltfModel` must be composed inside that `SceneCoreEntity`'s content lambda**: an empty lambda creates only an interaction-proxy node while the visible models are sibling nodes, so the system hit-tests the visuals but never dispatches to the proxy's `MovableComponent` / `InteractableComponent`. Its visible GLBs and `GltfModelEntity` interaction proxies must use printer-local offsets below the same node. Do not create an independent `activitySpace` / `scene.keyEntity` placement branch for the proxies: on a live Galaxy XR run on 2026-05-25 the build plate and model rendered, components logged as attached, but neither `MovableComponent` nor gizmo `InteractableComponent` received manipulation callbacks because hits belonged to the independent Compose model tree. Do not copy the panel shell's `-1.5 m` Z offset onto the workspace.
   - **Do not auto-mount transform gizmos on import.** Start `gizmoTool` at `Select`; the user can choose Move/Rotate/Scale after the preview is stable. On 2026-05-24, auto-starting in `Move` constructed three `GizmoDragHandle` GLBs immediately after the dragon preview attached and the Galaxy XR process aborted in `split_engine_bridge.cc:101` with `NOT_FOUND: Attempt to use unknown material instance id: 30`.
...
8. **`SubspaceModifier.movable()` is public as of XR Compose alpha13 (this app uses alpha14), but full authoring panels retain `MovablePanelWrapper`.** Its narrow SceneCore drag strip preserves scrolling within tall editing cards and supports the app's reset-to-authored-position action; a default full-panel movable target would capture those gestures. Full editing panels are arranged outside a reserved central build-plate corridor rather than implemented as `Orbiter`s: Android's `Orbiter` is intended for compact controls attached to a spatial parent such as a `SpatialPanel`, not clearance/following around this Compose-bridged SceneCore/`SpatialGltfModel` workspace. An orbiter remains appropriate for a future compact plate-local toolbar (lock/move, selection actions), not the project/settings/transform surfaces.
   - **Saved layout behavior:** A completed panel drag is saved in `UserPreferences` and becomes that panel's position on subsequent launches. `Reset Workspace` clears every saved panel position and returns to the authored first-launch layout as a recovery path for misplaced panels. The first-launch baseline for moved core panels was promoted from a live Galaxy XR arrangement captured on 2026-05-24; panels absent from that capture retain their existing clearance-oriented authored defaults.
...
10. **SceneCore parent detach cascades to children before Compose `DisposableEffect.onDispose` runs.** When the activity tears down, detaching a parent `Entity` (via `parent = null`) lets GC reclaim its child `GltfModelEntity`s, so each child's `onDispose { ent.removeComponent(ic); ent.parent = null }` then throws `Entity.DisposedException` and aborts `Activity.performDestroy()` — the next process bootstraps with empty state and looks like the user's work vanished. Wrap teardown in `if (!ent.isDisposed) runCatching { ... }`. Same applies to setPose racing with Filament's material binding on freshly-created entities: re-issuing `setScale(Vector3, Space.PARENT)` on every recomposition (vs once at create with the scalar overload) aborts the render thread with `split_engine_bridge: unknown material instance id`.
    ...
    **Alpha15 deprecations applied (2026-05-20):**
    ...
    - **Do not implement the visible selection outline with `CustomMesh` / `MeshEntity`.** That path was tried in May 2026: it required retaining mesh/material/direct-buffer resource graphs for process lifetime to avoid native borrow-finalization aborts, and after visible previews moved to `SpatialGltfModel` its SceneCore placement made the outline render at the wrong apparent size. `SpatialSelectionOutline` now writes a tiny orange outline GLB and feeds it through the same `SpatialGlbVisual` transform as the selected model.
...
11e. **`addComponent` queues an op against the entity's material instance — give Filament a few frames to bind it before triggering the IC attach.** Synchronous `entity = ent` immediately after `GltfModelEntity.create` + `applyWorkspacePose` lands the IC's collider attach on the render queue before the entity's material is bound, racing `split_engine_bridge.cc:100 NOT_FOUND: unknown material instance id: N`. Two `awaitFrame()` (gotcha #13's deferred dispose) is enough for the dispose direction but not for the create direction on heavy 3MF loads. Fix (2026-05-02): three `awaitFrame()` between create+pose and the Compose state assignment that triggers the DisposableEffect's IC attach. Applies to `GlbSceneEntity`. **TransformGizmo handles (Translate/Rotate/Scale, three call sites) need MORE than three awaitFrame** — when a 3MF first lands, gotcha #22's twin `writeColoredGlb` calls plus concurrent handle attachment can saturate Filament's bind queue. Fix (2026-05-04): (a) gate the TransformGizmo composable on `selectedModel.previewPath != null` at the call site so the gizmo doesn't compose at all until the model preview GLB has hit disk, and (b) inside each handle's `LaunchedEffect(session, filename)`, prefix a `delay(250)` BEFORE `GltfModelEntity.create`. The original 3-awaitFrame post-create gate stays. The first 3MF load post-fix on 2026-05-04 stopped crashing at `material instance id: 25` / `27`.
...
11g. **`StlReader` is binary-only, streaming-optimized to prevent JVM OutOfMemoryError, and has a fast `isBinary` probe.**
    - **Binary only / ASCII fallback:** `StlReader` reads bytes 80–84 as a triangle count; for ASCII STL these bytes are mid-`facet` text and the count comes out absurd (e.g. `1702130277`). `deriveStlFor` utilizes a microsecond-fast `StlReader.isBinary(file)` check that validates the binary header and exact expected file size without parsing the mesh. If it fails, it forces `SlicerEngine.convertToStl` (libslic3r round-trip) to emit a binary STL.
    - **Streaming/Chunked Parser:** To load heavy models (e.g. 1.4M-tri `dragon.3mf`), we stream the input in 204 KB batches (4,096 triangles at a time) using a `BufferedInputStream` and a reusable byte buffer instead of a full heap allocation (`InputStream.readBytes()`), completely avoiding JVM `OutOfMemoryError` during bed-collision/fit calculations.
    - **Multi-object 3MF Avoidance:** Multi-object 3MF parts already have their single-object STL extracted into `initial.source` during the 3MF preview pipeline. We reuse `initial.source` directly rather than invoking a redundant, heavy JNI merge of the entire 3MF container.
    - **OOM Catching:** Normal `runCatching` blocks do not catch `java.lang.Error` subclasses such as `OutOfMemoryError` (which bypasses `Exception`). Wrap critical mesh-checking blocks in explicit `try-catch` including `OutOfMemoryError` so that any unexpected heap exhaustion degrades gracefully rather than crashing the Android process.

11j. **A single NaN coordinate in any child's transform silently culls the entire parent graph (including sibling entities like the build plate) in SceneCore/Filament.** A bug in `slic3r_jni.cpp` `nativeRead3mfObjectMetadata` allowed empty or invalid STL bounds to propagate `NaN` or `inf` into the returned `translateXmm` / `translateYmm` offsets. When Jetpack XR `alpha15` received these in `GltfModelEntity.setPose()`, the underlying Filament node recalculated its bounding box to `NaN`. Because the parent `WorkspaceEntity`'s bounding box is the union of its children, the `NaN` poisoned the entire workspace bounds, causing the frustum culler to silently drop the entire sub-tree. Symptom: loading a broken model makes *both* the model and the static `build_plate.glb` disappear simultaneously. Fix: always apply `std::isfinite` sanitization (falling back to 0.0) to raw bounding box values in JNI before using them in vector offsets or returning them to Kotlin.
...
13. **Synchronous dispose of a `GltfModelEntity` mid-swap aborts the render thread with `split_engine_bridge.cc:100` `NOT_FOUND`.** Two flavors observed: `"unknown material instance id: <N>"` and `"unknown node: <N>"`. Both come from queued render-thread ops (setPose, addComponent for an Interactable/Movable collider) that reference a node id whose entity was just freed. Two patterns produce this:
...
15. **Use `SpatialGltfModel` for visible generated GLBs, but never as a separate transform/input tree.** On Galaxy XR, `GltfModelEntity` can load, attach, report a real-world pose within field of view, and still produce no visible plate/model/toolpath. The official XR sample renders through `SpatialGltfModel`; OrcaXR mirrors that route with `ByteArraySpatialGltfModelSource` while retaining `GltfModelEntity` proxies for `InteractableComponent` / `MovableComponent`. Both paths must be descendants of the same Compose-bridged workspace `SceneCoreEntity`, and `SpatialGlbVisual` transforms must remain workspace-local so a bed move or live gizmo delta updates what the user sees as well as the proxy. A parallel standalone proxy tree is rendered-but-inert on Galaxy XR (verified 2026-05-25). The debug-only probe (`SpatialGltfProbeActivity` + `SpatialGltfProbeTest`) remains the lifecycle stress test.

16. **Transform Gizmos (Move/Rotate/Scale) must be rendered visually using `SpatialGltfModel` while retaining `GltfModelEntity` as interaction proxies.**
    On Galaxy XR, raw `GltfModelEntity`s parented to the workspace (`OrcaXR-workspace`) are excellent interaction/collision proxies for `InteractableComponent` pinches and lasers, but they are visually invisible (they do not produce visible geometry). To make the Move/Rotate/Scale gizmo handles visible, their visual representation must be composed via `SpatialGltfModel` (using `SpatialGlbVisual`) in the Compose subspace tree. Since `SubspaceModifier.scale` only supports uniform scale (a single `Float`), we bake the exact non-uniform length/thickness dimensions of each handle into its generated GLB file dynamically in millimetres (using distinct filenames like `arrow_x_${len}_${radius}.glb`), allowing both the visible `SpatialGltfModel` and the raw proxy `GltfModelEntity` to be scaled uniformly by `WORLD_SCALE = 0.0015f`. This completely avoids non-uniform scale races on Filament binding and ensures visual elements perfectly align with their active interaction proxies.
