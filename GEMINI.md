...
20. **FullSpectrum mixed-filament integration: data-model + wire format are at FS v0.9.9 parity, engine emission is NOT ported.** As of 2026-05-10 the patches 0015-0024 land all data structures (`MixedFilament.{cpp,hpp}`, `filament_mixer*`, `LocalZOrderOptimizer.hpp`), register the 17 PrintConfig keys (`mixed_filament_*`, `dithering_*`, `local_z_wipe_tower_purge_lines`), and add the `MixedFilamentManager` member on `Print`. The Kotlin side (`MixedFilamentEntry`, `MixedFilamentStore.toMixedFilamentDefinitions`, `parseMixedDefinitionsForKotlin` in MainActivity, `OrcaProfileLoader.SAFE_KEYS`, `SlicerEngine.PROJECT_OVERRIDE_KEYS`, `MixedAdvancedEditor` UX, MCP `set_mixed_filament_row` / `delete_mixed_filament_row` / `list_mixed_filaments`) all speak the v0.9.9 wire format and round-trip cleanly. **Patches 0027-0060 landed + verified compile-clean 2026-05-11.** Every FS file delta NOT entangled with the Local-Z planner has been ported:
  ...
  - 0060 `Print.cpp` `collect_local_z_wipe_tower_toolchanges` + wipe-tower planning hook (LZ5e, 530 LoC): adds an anonymous-namespace block with `LocalZWipeTowerToolchange` + `LocalZWipeTowerPassRef` structs, perimeter intersection probes (`layer_has_local_z_perimeters`, `extrusion_collection_has_local_z_perimeter_segment`), small extruder-ordering helpers (`shared_local_z_extruder_for_wipe_tower`, `rotate_extruders_to_start_with`), and the ~325-LoC `collect_local_z_wipe_tower_toolchanges` itself — two modes: (1) legacy print_z-group + `LocalZOrderOptimizer::order_bucket_extruders`, (2) dependency-chain topological scheduler when every SubLayerPlan has dependency_group/_order. Wires `collect_layers_to_print` → `collect_local_z_wipe_tower_toolchanges` → `WipeTower2::plan_local_z_toolchange` into the wipe-tower planning loop's WipeTower2 branch. Adds first `#include "LocalZOrderOptimizer.hpp"` call site (header was landed in patch 0023). **With patch 0060, Local-Z purges are actually PLANNED through the wipe tower** — `WipeTowerData::local_z_tool_changes` + `local_z_reserve_boxes` populate with non-empty values for Local-Z prints. **What remains:** LZ5f — GCode.cpp call sites that invoke `WipeTowerIntegration::tool_change(extruder, finish_layer, /*local_z_unplanned=*/true, sub_z)` between sub-Z extrusions (the Z-walk emit loop that scans `PrintObject::local_z_sublayer_plan()` records — FS GCode.cpp lines 4958-5800+).
