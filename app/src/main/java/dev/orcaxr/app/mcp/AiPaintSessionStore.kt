...
internal class AiPaintSession internal constructor(
    ...
    // ---- M7: paint constraints ----

    private val constraints = java.util.concurrent.CopyOnWriteArrayList<AiPaintConstraint>()

    /** Append a new constraint. Constraint id is generated by the
     *  caller (the MCP tool layer); we don't enforce uniqueness, but
     *  by convention the tool generates a UUID-style hex string. */
    fun addConstraint(c: AiPaintConstraint) {
        constraints.add(c)
        lastTouchedAtMs = System.currentTimeMillis()
    }

    /** Snapshot of all constraints attached to this session, in
     *  insertion order. */
    fun listConstraints(): List<AiPaintConstraint> = constraints.toList()

    /** Drop the constraint with the given id. Returns true iff one
     *  was removed. */
    fun removeConstraint(id: String): Boolean {
        val it = constraints.iterator()
        while (it.hasNext()) {
            if (it.next().id == id) {
                // CopyOnWriteArrayList.iterator does not support
                // remove(); call removeAll on a single-element list.
                return constraints.removeAll { it.id == id }
            }
        }
        return false
    }
    ...
    /** Walk every attached constraint against the current
     *  `paintFilamentIndex`. Returns an empty list when the session
     *  is OK to commit; otherwise the failures, sorted by insertion
     *  order. */
    fun validateAttachedConstraints(): List<ConstraintViolation> =
        validateConstraints(paintFilamentIndex, triCount, constraints.toList())
