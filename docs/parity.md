...
- [~] **P11.2 — Close every menu, toolbar, context, camera, and view gap.** File, Edit, View, Add,
  ...
  - **Missing:** the P0 surface-manifest classification pass is untouched, and neither the outline
    nor the navigator has had a visual review. The XR context menu is a **selection-scoped section**
...
| Truthful menu/toolbar/context/shortcut/XR capability state | P0.2, P11.2 | One guarded registry reports 154 actions = 112 partial + 42 unavailable and zero implemented, with unique real task ownership/anchors, and context menus are now derived from the same catalog for both the scene and the Objects tree; full upstream reachability and the P0 manifest classification remain |
