...
- [~] **P5.3.5 — Simplify UI.** Front the existing guarded topology-replacement command with the
  ...
  - **Outstanding:** upstream's per-object configuration persistence and XR remain — all three
    preview actions are DOM-only, so the pinned gizmo is not yet reachable in a headset.
...
- [~] **P5.3.6 — Brim ears.** The placement gizmo P4.8 deferred: pick anchor points, author the
  ...
  - **Outstanding:** hover and selected ear states are not exposed (the port carries their pinned
    colours but nothing drives them yet), and all five actions declare an `xrUnsupportedReason`.
