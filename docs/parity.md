...
- [~] **P5.3.5 — Simplify UI.** Front the existing guarded topology-replacement command with the
  ...
  - **Outstanding:** upstream's per-object configuration persistence and XR remain, and the
    preview's DOM surface is not built — the session is reachable through the workspace API and the
    existing one-shot `simplify_model` action, not yet through preview/apply/cancel controls.
