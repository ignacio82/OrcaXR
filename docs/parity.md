...
- [~] **P9.6 — Add camera, console, macros, and history.** Discover webcam endpoints; render
  ...
  - **Current:** the console is live, and the interesting work in it is knowing what a command does
    ...
    to, in three readings: Moonraker's own origin is a path fetched through the credentialed
    transport; the same host on another port — the ordinary crowsnest arrangement, Moonraker on
    7125 and the camera on 8080 — is fetched directly and deliberately **without** the API key,
    because that service never issued one; and another host entirely is refused with both origins
    named, so printer-host content cannot turn the page into a request forwarder. Stripping the
    origin off every absolute URL, which is what this did until 2026-08-27, silently re-pointed the
    second case at Moonraker's port, where it answers nothing and the panel waits for a frame that
    is never coming. A rotation or flip the camera declares is reproduced so the picture matches how
    the machine is actually mounted.
  - **Missing:** the console, macros, history, and camera are DOM only; XR has none of them. Live
    ...
    A camera on another port is only as reachable as its own CORS policy allows — LNA does not
    bypass CORS — so a service that refuses cross-origin reads is reported as unreachable with that
    named as a cause, which is a diagnosis rather than a picture.
...
| `ADAPT-12` | A live MJPEG, WebRTC, or HLS camera feed | Snapshots at up to 4 fps, paused whenever nobody is watching: through the credentialed transport when Moonraker serves them, and directly *without* the API key when the camera is a separate service on another port of the same machine | The picture updates a few times a second instead of continuously, and a camera that offers only a stream is listed with that reason rather than shown | An `<img>` or `<video>` pointed at the printer cannot send `x-api-key`, so on a secured printer the feed would be a broken image; making it load would mean putting the key in the URL, which the transport refuses. Snapshots keep the credential in a header and work for every service type | Engineering | P9.6, P10.7, hardware camera qualification | Platform-adapted / proposed |
