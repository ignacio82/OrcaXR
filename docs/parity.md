...
- [~] **P8.3 — Implement all generic calibration generators and instructions.** Provide parameter
  ...
  - **Panel and contextual docs (2026-08-17):** `ui/dom/CalibrationParametersPanel.ts` renders the
    ...
    that do not exist.
  - **Missing:** the parameter panel is not mounted in the shell yet, and has no XR surface. Slice,
    inspect, and export/send for the generic modes are not built. The saved preset is
