...
- [~] **P12.3 — Run engine and artifact qualification.** Rebuild WASM, browser assets, server
  ...
  - **Not qualified here, and the tool says so in its own output:** rebuilding the WASM engine or
    the server CLI from clean sources needs the Emscripten toolchain. And **route comparison has no
    suite at all** — the other seven corpora P12.3 names are covered, and listing only those would
    have read as full coverage of the list, so the absent one is a deliberate entry rather than an
    omission.
