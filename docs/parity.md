...
- [~] **P6.5 — Reuse the same schema at every scope and surface.** Project, plate, object, part,
  ...
  - **The stepper that surface needs exists (`EVID-120`):** `settings/editor/stepper.ts` derives one
    press of change from an option's own bounds, because — unlike a calibration parameter — an
    engine option declares a range but **no step**. A hundredth of the span, snapped to a round
    power of ten, so what an operator reads is 0.1 rather than 0.137. Booleans toggle and enums
    cycle. **An option with no bounds is refused**, the rule already applied to the brim-ear radius
    and the SVG size: a stepper without limits is a typed field with extra presses and would let a
    headset reach a value the DOM would refuse. Vectors are refused because no component was named,
    and `float-or-percent` because its bounds do not say whether they constrain `50` or `50%` —
    stepping one as the other silently rewrites the setting's meaning.
