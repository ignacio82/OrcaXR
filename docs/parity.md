...
- [~] **P5.3.6 — Brim ears.** The placement gizmo P4.8 deferred: pick anchor points, author the
  ...
  - **Slice finding (2026-08-16):** the Accept clause was finally asked, by driving the pinned
    WASM engine headlessly — the first test in this repo to do so. The first half holds: the
    archive carries `Metadata/brim_ear_points.txt` in the pinned format, keyed by the 1-based
    model-object index the pinned reader resolves (`bbs_3mf.cpp:2021`), and `brim_type` reaches
    the engine. The second half does **not**: the sliced result contains no brim at all, and the
    first layer extrudes identically with and without ears.
    Two things were corrected along the way. `brim_type` must be `painted` (`btPainted`) for
    *placed* points to be consumed; `brim_ears` (`btEar`) is the automatic corner detector and
    ignores them (`Brim.cpp:929-930`), so the obvious-looking value is the wrong one. And the
    failure is silent — a clean slice, no error, no brim — which is precisely why an assumption
