...
- [~] **P7.8 — Author layer custom-G-code events and filament sequences.** On the layer slider,
  ...
  - **Current:** authored events are canonical, guarded, and reach the engine. Each carries the
    ...
    Authoring on the layer slider itself, event ticks and badges in the preview, filament
    sequences, MultiAsSingle tool-change events, and template bodies remain.
...
- [~] **P9.5 — Implement live Status and Monitor outcomes.** Show state, file/thumbnail, overall
  ...
  - **Current:** a typed live model reads exactly the Klipper objects it declares
    ...
    stale "printing" readout. Thumbnails, fans, speed/flow, axes/homing, load/unload, lights,
    storage browsing, and update-manager surfaces remain.
...
| Layer pauses/custom G-code/color changes/filament sequences | P7.8 | Guarded canonical authoring, an engine-format `custom_gcode_per_layer.xml` projection that round-trips foreign projects, and profile-gated event kinds are live; slider authoring, preview ticks, and filament sequences remain |
