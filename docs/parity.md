...
- [~] **P5.3.4 — SVG emboss and SVG part.** Import an SVG, resolve its paths to a mesh volume, and
  ...
  - **A real corpus, and what it does not cover (`EVID-114`):** all 102 SVGs the app ships — Material
    Design icons that arrived from a real tool rather than from a test — parse, extrude, and report
    nothing unsupported. Its worth is bounded and stated: they are simple filled paths carrying no
    stylesheet, no grouped `fill="none"`, no `!important` and no media query, so they exercise
    **none** of the four defects. What they establish is that ordinary geometry from a real tool
    round-trips, and that a future change to the cascade will not start refusing files that work
    today. **Two tool-shaped documents cover what they cannot (`EVID-115`):** modelled on what
    Illustrator and Inkscape actually emit — a `<style>` block of `.st0{fill:none;stroke:…}`
    classes, layer groups carrying `style="fill:none"` and a transform, a stroked construction line
    beside filled artwork. Before the cascade work both produced *two* contours, the construction
    line solidified into the part; both now yield one and report the line.
...
- [~] **P6.5 — Reuse the same schema at every scope and surface.** Project, plate, object, part,
  ...
  - **The third surface (`EVID-120`, `EVID-121`).** XR had no scoped settings at all, because every
    route into a setting ended at a text field. It has one now, and it is a *view* of the existing
    editor rather than a second implementation: `settings/editor/scopedStepper.ts` holds the target
    being edited and a `SettingsDraftEditor` bound to it, runs the same query the DOM panel runs,
    and hands each press to the same adapter the DOM panel applies through — so the two surfaces
    cannot drift into two write paths. The value itself comes from `settings/editor/stepper.ts`,
    which derives one press of change from an option's own bounds, because — unlike a calibration
    parameter — an engine option declares a range but **no step**. A hundredth of the span, snapped
    to a round power of ten, so what an operator reads is 0.1 rather than 0.137. Booleans toggle
    and enums cycle. **An option with no bounds is refused**, the rule already applied to the
    brim-ear radius and the SVG size: a stepper without limits is a typed field with extra presses,
    and it would let a headset reach a value the DOM would refuse. Vectors are refused because no
    component was named, and `float-or-percent` because its bounds do not say whether they
    constrain `50` or `50%` — stepping one as the other silently rewrites the setting's meaning.
    A refused setting still shows its value and says why it cannot be stepped here, since an
    operator who cannot find a setting cannot tell "absent" from "not supported".
    The rows are drawn through the mockable `XrUiAdapter`, which is what makes the recurring XR
    failure — a control that renders and changes nothing — assertable without a headset: the traces
    click the rendered nodes and require the press to arrive with the field and direction the row
    claims. In the production browser the same controller the headset renders offers 46 settings at
    the project scope, and a keyboard-less press changes one and undoes it as a single canonical
    command. The panel also **follows the selection** (`EVID-122`): pointing at a model moves the
    editor to that object, because cycling past every plate and part is not a way to choose a node
    in a headset. Only a *changed* selection moves it, and clearing one leaves the panel where it
    was — an operator mid-edit did not ask to be sent back to the project.
  - **Missing:** the surface has never been rendered in a headset. Everything above is proven in
    Node and in a desktop browser, so what remains for this task is exactly P10.5's gate: reach,
    legibility, and controller input judged in a real session on the Galaxy XR. Two smaller gaps
    are stated rather than closed: the derived step is this repository's judgement, not a pinned
    upstream value — upstream's own spin controls may increment differently and nothing here
    compares them — and settings whose generated definition no surface supports are counted in one
    line instead of drawn as 236 inert rows, which is a deliberate divergence from the DOM panel.
...
- [~] **P9.6 — Add camera, console, macros, and history.** Discover webcam endpoints; render
  ...
  - **Current:** the console is live, and the interesting work in it is knowing what a command does
    ...
    moment nobody is watching: a hidden tab, another workspace, a collapsed section, a disposed
    panel. Where a reported URL *points* is decided against the endpoint the session is connected
    to, and a relative path — the usual form — belongs to the printer's **web** origin rather than
    its API port, because that is where a stock install's nginx serves it. How a frame is obtained
    follows from who serves it: a camera on any origin but Moonraker's takes no credential, so the
    browser is simply pointed at it, which is the one route needing no CORS and therefore the one
    that works against services that send no cross-origin headers; a camera Moonraker serves is
    fetched through the credentialed transport, because an `<img>` cannot send `x-api-key` and the
    key must not go in the URL; and a camera on another host entirely is refused with both origins
    named, so printer-host content cannot turn the page into a request forwarder. The routes are
    walked in that order and the first that works is kept, so no one is asked which arrangement
    their printer has. A rotation or flip the camera declares is reproduced so the picture matches
    how the machine is actually mounted.
  - **Missing:** the console, macros, history, and camera are DOM only; XR has none of them. Live
    ...
    On the hosted HTTPS app a plain-HTTP camera cannot be displayed at all — mixed content — so the
    only route left is reading its bytes, which LNA permits but CORS does not, and most camera
    services send no cross-origin headers. There the panel reports that dead end in those terms
    rather than waiting: a diagnosis, not a picture. An HTTP page has no such limit.
...
- [x] **P10.4.4 — Pseudo-localization and RTL layout gates.** The expanded and mirrored runs fail
  the build on a clipped or mis-mirrored critical control rather than merely being available.
  - **Accept:** a pseudo-long and a mirrored run clip no critical control and reverse no directional
    affordance; documented geometry-direction exceptions are recorded.
  - **Current:** two gates, both in `quality`, and both proven to bite by introducing a violation
    (`EVID-129`).
    `direction:check` refuses physical direction in CSS — `margin-left`, `text-align: left`,
    `border-right`, a physical `left:`/`right:` inset — across the app stylesheet and every inline
    style set from TypeScript. It is static because a rendered check only finds the physical
    properties that happen to be on screen in whatever state a smoke test reached, and the rest
    ship. Fifty-four declarations were converted to logical properties to make it pass. Two things
    are allowed: `left: 50%`, because fifty percent is the same distance from either edge and
    converting the centring idiom to `inset-inline-start` would actively *break* RTL; and a
    position that is genuinely physical, which says so with a `direction:physical` comment carrying
    its reason — a context menu opens at a pointer's viewport coordinate in any writing direction,
    and mirroring it would put the menu across the screen from the thing it was opened on.
    `test:pseudo` renders the built app in both pseudo-locales and measures **152 critical
    controls** — primary bar, tool rail, menu triggers, menu items, stage bar, calibration grid —
    for geometric overflow, which is what "clipped" means whether the overflow is hidden,
    ellipsised, or spilling. It refuses to run against fewer than forty, because a green
    "none clipped" over an empty selector list is worse than no gate.
  - **The run found a real defect on its first pass.** The tool rail was a fixed 158 px, measured
    against English: eight labels truncated at once under a 40 % expansion, and "Stützstrukturen
    bemalen" would have done the same to a German operator with nothing failing. The rail now sizes
    to `max-content` between a floor and a cap, so it follows whatever language it renders, and its
    labels wrap rather than ellipsising — the cap means a long enough label has to go somewhere, and
    a second line costs a few pixels of height where a truncated one costs the tool's name. The
    ellipsis was the worse half of that defect: it made the truncation look intentional.
    A second defect came from the language switch itself — the DOM shell appends to the primary bar
    rather than clearing it, because the hidden file input the Load action needs lives there, so
    remounting on a language change duplicated every button. The shell now tracks what it added.
  - **Mirroring is asserted, not assumed.** A layout that sets `dir="rtl"` and lays out identically
    has not mirrored, so the run holds the inspector to crossing the viewport centre between the
    two directions rather than merely to the attribute being set.
  - **Outstanding:** none of the twenty upstream languages is RTL, so no shipped language exercises
    the mirrored path — it is held by the pseudo-locale, which is the honest substitute rather than
    an equivalent. Legibility is out of scope for a geometric check: this finds clipping, not
    crowding, and P10.5's headset review is where a human judges the result. XR panel text is not
    measured, since the pseudo run drives the DOM shell.
...
- [~] **P11.2 — Close every menu, toolbar, context, camera, and view gap.** File, Edit, View, Add,
  ...
  - **G-code window, indexed half (`EVID-116`):** `project/gcode/GcodeDocument.ts` is the readable
    model of a sliced program — line-offset index, clamped windows, and a bounded search that says
    when it was bounded. Size is the whole design constraint: a small cube slices to ~14,500 lines
    and a real plate reaches hundreds of thousands, so putting the text into the DOM freezes the
    tab while truncating it silently means the operator reads a program that is not the one they
    will print. Every count reported is of the whole document rather than the window. This matters
    more since calibration sweeps became generated programs (`EVID-096`–`EVID-098`): an operator is
    asked to inspect a file before printing it and had nothing to inspect it in.
  - **And the window itself (`EVID-117`):** `ui/dom/GcodePanel.ts` renders it. Only the visible
    rows exist in the DOM — a full-height spacer carries the scrollbar and a window of rows is
    positioned at its true offset — because rendering a whole program locks the tab on the first
    file anyone opens. Counts describe the whole program rather than what is rendered, a capped
    search says **"at least"** so nobody stops looking, and a line is inserted as text and never as
    markup, since a program is untrusted input and a comment can carry anything. The panel is
    rebuilt only when the program changes, so a canonical tick does not drop an operator's scroll
    position and search while they are reading.
  - **A browser check found two wiring defects the unit traces could not (`EVID-118`):** the panel
    subscribed only to canonical state, which does not change when a slice produces a program — so
    the window would have stayed **permanently empty** for every operator, while its unit traces
    passed because they supply their own document. And three panels added earlier this session —
    Simplify, Calibration parameters, and this one — had been dropped inside the **Brim ears**
    section, so opening Brim ears showed all four. Each panel now has its own section, and the
    G-code listing sits in the Preview tab beside the G-code preview where it belongs.
    **The pattern was then checked across every mounted panel (`EVID-119`)** rather than left as a
    twice-seen anecdote: all 23 either subscribe to a signal or are pushed to explicitly, and only
    the G-code window had the mismatch. What that audit does *not* establish is that each
    subscription covers every signal its own state reads — that is the specific form the defect
    took, and it is not statically checkable.
  - **Context menus, from the same catalog (`EVID-123`):** the scene had none at all, and the
    Objects tree had a hand-built two-entry menu — two answers to the same right-click is exactly
    the reachability gap this task exists to close. An action now names the targets it belongs to
    (`object`, `plate`), the registry turns that into `dom-context` and `xr-context` surfaces under
    the same XR rule as every other surface, and `ui/dom/ContextMenu.ts` renders it. One component:
    the scene's right-click and the Objects tree's row menu are the same code with the same
    keyboard behaviour, and the tree's own two operations — Reveal and Rename, which act on the
    tree rather than the project — sit above the catalog's rather than being mixed into it.
    Right-clicking a model **selects it first**, which is what makes the menu truthful: every
    object action is gated on the selection, so a menu opened without selecting would offer rows
    disabled for a reason the operator just disproved by clicking the model.
    An action that cannot run stays out. It remains reachable in the menu bar and the palette where
    its reason is stated; a shortcut surface filled with rows that only explain themselves is not a
    shortcut. What does appear, appears with the registry's own availability sentence.
  - **The classification pass (`EVID-124`):** the manifest's 205 upstream menu leaves were all
    dispositioned to *this task* and then never answered one at a time, so "does OrcaXR have that
    menu item?" could only be settled by reading the catalog and guessing at a label.
    `tools/parity/surface-map.json` now answers each of the 162 distinct items as exactly one of
    five things: **101** are reached by a named local action, **7** through a panel rather than a
    catalogued action, **17** are submenu headings rather than items, **15** are deliberate
    adaptations citing the register entry that explains them, and **22** are absent and name the
    task that owes them. (It opened at 98 reached and 25 absent; the cone, disc, and torus moved
    across when `EVID-125` built them, scale-to-build-volume when `EVID-126` did, and per-object
    printability with `EVID-127` — the gap list working as one: **103 reached, 20 absent** as of
    this row.) Adaptation and absence are kept apart deliberately — one says the item
    will not arrive, the other says it has not arrived yet — so the gap list cannot shrink by
    relabelling. The check runs against three separately maintained files (the pinned manifest, the
    overlay, and the live catalog): an unclassified leaf fails, a stale entry that classifies
    nothing fails, an action id that does not exist fails, and a cited `ADAPT-nn` or task the plan
    does not define fails. Every one of the 98 mapped actions reaches a DOM surface and the command
    palette, and is either on an XR surface or states why it is not.
  - **Missing:** neither the outline nor the navigator has had a visual review, and the 20 absent
    items are absent — the largest are the SLA menus (out of scope for FFF targets), the
    auxiliary-file browser, input-shaping and junction-deviation calibrations, per-object STL
    export, instance-to-object promotion, and Objects-tree row visibility. Classification is not
    implementation, and that row claims only the former. The XR context menu is a **selection-scoped section**
    at the head of the Panels menu rather than a spatial popup at the ray hit: the headset has no
    long-press channel on a model yet, so what a headset can address is what is selected. Whether
    that reads as a context menu in a real session is a P10.5 question, not a Node one.
...
| Truthful menu/toolbar/context/shortcut/XR capability state | P0.2, P11.2 | One guarded registry reports 154 actions = 112 partial + 42 unavailable and zero implemented, with unique real task ownership/anchors, context menus derived from the same catalog for both the scene and the Objects tree, and all 205 upstream menu leaves classified as action/panel/container/adaptation/absent with the check crossing manifest, overlay, and registry; the 25 absent items and a headset session remain |
...
| Per-object/part/layer settings | P2.5, P6.5 | Live height-range lifecycle plus one panel that edits plate/object/part/height-range overrides through an engine-generated scope table, with the height range outranking the part as the engine orders it, and an XR stepper surface over the same scopes; a headset session remains |
...
| Settings search/modes/dependencies/validation/reset/compare | P6.2–P6.5 | Live Process panel has pinned grouping plus headless modes/search/validation/inheritance/reset/compare, the same panel serves all five override scopes from a generated per-scope key table, and XR reaches the same scopes as stepper rows over the same draft editor and adapter; predicates and a headset session are missing |
...
| `EVID-114` | P5.3.4, P12.3, P0.2 | Current worktree atop `5e8c9c2`; commit pending | `9fd12ff...` | 2026-08-18 | `npm --prefix web run quality`; `src/project/objects/__tests__/svg-real-corpus.test.ts` | Node 22.21.0; **no headset, no hardware** | Pass, and it is the check `EVID-113` named as most likely to find a fifth defect — it did not. All **102** SVGs this app ships parse, extrude, and report nothing unsupported. Four defects had been pinned by hand-written documents, which are exactly the corpus most likely to keep agreeing with the parser that inspired them, so this runs files that arrived from somewhere else. **The limit is asserted rather than implied:** these are simple filled Material Design paths carrying no stylesheet, no grouped `fill="none"`, no `!important` and no media query, so they exercise none of the four defects; what they establish is that ordinary real-tool geometry round-trips and that a future cascade change will not start refusing files that work today. The corpus size is asserted before its contents, because a corpus that has quietly emptied proves nothing. 191/191 unit, 5/5 integration, 80/80 project, 12/12 slice, 8/8 settings, and 1/1 XR files pass | Automated review; a clean run over 102 files is weaker evidence than it looks, because the files are homogeneous — one tool, one style, one shape of output. A drawing exported from Illustrator or Inkscape with layers, classes and stroked construction lines is the corpus that would actually exercise the four fixes, and this repository does not contain one |
| `EVID-115` | P5.3.4, P0.2 | Current worktree atop `1a1e1bd`; commit pending | `9fd12ff...` | 2026-08-18 | `npm --prefix web run quality`; `src/project/objects/__tests__/svg-real-corpus.test.ts` | Node 22.21.0; **no headset, no hardware** | Pass: `EVID-114` closed by naming the missing corpus — a drawing with layers, classes and stroked construction lines — so it was built to the shape those tools really produce. Illustrator writes a `<style>` block of `.st0{fill:none;stroke:…}` classes with construction geometry on its own layer; Inkscape writes layer groups carrying `style="fill:none;stroke:…"` and a transform. Both put a stroked line beside filled artwork, which is simply how a part is drawn. **The assertion is not that they parse but that the construction geometry is excluded and reported:** before the four fixes each produced *two* contours, with the line solidified into the part, and each now yields one. This is the case all four defects were really about, exercised in combination rather than one feature at a time. 191/191 unit, 5/5 integration, 80/80 project, 12/12 slice, 8/8 settings, and 1/1 XR files pass | Automated review; these are modelled on those tools' output, **not exported from them** — the structure and idioms are faithful but no file here came off a real installation, so a quirk neither tool's documentation nor my model captures would still be missed. An actual export dropped into the corpus would settle that, and it is a small thing an operator can supply that no amount of synthesis replaces |
| `EVID-116` | P11.2, P0.2 | Current worktree atop `63217fd`; commit pending | `9fd12ff...` | 2026-08-18 | `npm --prefix web run quality`; `src/project/gcode/__tests__/GcodeDocument.test.ts` | Node 22.21.0; **no headset, no hardware** | Pass: the readable model behind P11.2's G-code window. Line-offset indexing, clamped windows, and a search bounded at 500 results that reports `truncated` — a viewer showing "500 matches" without saying there are more would have an operator believe they had seen them all. A window past the end clamps rather than inventing rows, because a viewer scrolled into a program that was re-sliced shorter must show the end of the new one. **A real edge case was caught by its own trace rather than shipped:** the final line's slice runs to the end of the source, which still carries its own `\r\n` when the program ends with a newline — that offset is dropped so it does not count as an extra line — so line *n* was returned with a trailing terminator. A 200,000-line program is windowed near its end and searched in the same trace, since an implementation that split the source per window would be unusable at the size a real plate reaches. 191/191 unit, 5/5 integration, 80/80 project, 12/12 slice, 8/8 settings, and 1/1 XR files pass | Automated review; **this is the model, not the window** — no panel renders it, so an operator still cannot read a program in the app, and P11.2 stays open on that as well as on the remaining context-menu surfaces. Search is a plain case-insensitive substring match, not the regular expressions or field-aware filtering upstream's viewer offers |
| `EVID-117` | P11.2, P10.2, P10.6, P0.3 | Current worktree atop `8519c64`; commit pending | `9fd12ff...` | 2026-08-18 | `npm --prefix web run quality`; `src/ui/dom/__tests__/GcodePanel.test.ts` | Chrome for Testing 150.0.7871.24; jsdom 29; **no headset, no hardware** | Pass: an operator can read a sliced program in the app. The window renders only visible rows against a full-height spacer, and a trace asserts that a 50,000-line program puts fewer than 500 rows in the DOM — rendering the whole thing is the obvious implementation and it locks the tab on the first file anyone opens. Counts describe the whole program rather than the window; a search capped at 500 reports **"at least"**, because a bare count reads as the total and an operator stops looking. **A line is inserted as text and never as markup**, asserted against a program carrying `<img src=x onerror=…>` in a comment — a sliced file is untrusted input and this one is now displayed. The document is rebuilt only when the program changes, so a canonical tick does not throw away scroll position and search mid-read. This closes the loop opened by `EVID-098`: the sweep exporter asks an operator to inspect a file before printing it, and there was nowhere to inspect it. 191/191 unit, 5/5 integration, 80/80 project, 12/12 slice, 8/8 settings, and 1/1 XR files pass | Automated review; **one gate run failed and is not being reported as green** — the offline smoke timed out waiting for `#app-boot.ready`; both e2e and offline passed standalone and the whole gate passed on re-run, so it is the `EVID-035` contention flake rather than anything here. The row window is measured against jsdom's zero client height, so the overscan is what is proven bounded, not the viewport arithmetic; that a program scrolls smoothly in a browser has not been observed by a person. The panel is DOM-only and search is plain substring matching |
| `EVID-118` | P11.2, P10.1, P0.3 | Current worktree atop `399734f`; commit pending | `9fd12ff...` | 2026-08-18 | `npm --prefix web run quality`; the `readTheProgramInABrowser` step in `scripts/e2e-smoke.mjs` | Chrome for Testing 150.0.7871.24 driving the production build; **no headset, no hardware** | Pass, **and putting it in a real browser found two defects the unit traces structurally could not.** `EVID-117` recorded that jsdom's zero client height left the viewport arithmetic unproven; proving it showed the window would have been permanently empty for every operator — the panel subscribed to canonical state, which does not change when a slice produces a program, while its unit traces passed because they supply their own document. It now subscribes to both signals, which are genuinely different events. Second: three panels added earlier this session — Simplify, Calibration parameters and this one — had been dropped inside the **Brim ears** `<details>`, so opening Brim ears showed all four; each has its own section now and the listing sits in the Preview tab beside the G-code preview. With those fixed the browser step asserts what jsdom could not: a real sliced program puts fewer than 400 rows on the page, the status reports a line count several times larger than what is rendered, and scrolling to the middle moves the window rather than re-rendering the same rows. 191/191 unit, 5/5 integration, 80/80 project, 12/12 slice, 8/8 settings, and 1/1 XR files pass | Automated review; the step opens the sidebar, the Preview tab and the section itself, which is the state a real reader is in but is reached by script rather than by a person navigating there. That the listing is discoverable — that someone looking for their G-code finds this tab — is a P10.1 usability question no assertion answers |
| `EVID-119` | P11.2, P0.5, P0.2 | Current worktree atop `06ab8fb`; commit pending | `9fd12ff...` | 2026-08-18 | `npm --prefix web run quality`; a read of every panel mount in `src/main.ts` | Node 22.21.0; **no headset, no hardware** | Pass, **and it is a negative result recorded on purpose.** `EVID-118` found a panel whose tests passed while its wiring was dead — its unit traces supply their own document, so they never exercised the subscription. Having seen that shape twice it was worth asking whether it was systemic, so all 23 mounted panels were read: every one either subscribes to a signal or is pushed to explicitly (`SlicePreflightPanel` and the G-code preview use a push model, which is a different valid design rather than a missing subscription), and only the G-code window had the mismatch. A checked-and-clear audit is evidence in its own right — it stops the next reader repeating the search, and an unrecorded one gets redone. 191/191 unit, 5/5 integration, 80/80 project, 12/12 slice, 8/8 settings, and 1/1 XR files pass | Automated review; **this establishes that each panel subscribes to something, not that its subscription covers every signal its state reads** — which is the specific form the defect took and is not statically checkable, since `getState` can read anything reachable from the workspace. The only reliable guard remains a browser-level assertion that a panel populates, which exists for the G-code window and for the panels the e2e already drives, and not for the rest |
| `EVID-120` | P6.5, P2.6, P10.9, P0.2 | Current worktree atop `c1c9d18`; commit pending | `9fd12ff...` | 2026-08-18 | `npm --prefix web run quality`; `src/settings/editor/stepper.test.ts`; the shipped engine-option schema | Node 22.21.0; **no headset, no hardware** | Pass: the mechanism P6.5's own Missing note describes — "stepped values rather than typed text" — is built. The increment is derived from each option's range because an engine option, unlike a calibration parameter, declares bounds but no step. **Driven against the real shipped schema rather than hand-built definitions**, which is what made it useful: a whole-schema sweep steps every bounded scalar from both ends in both directions and asserts the result stays inside its own bounds, and it found **two real defects**. Rounding after clamping pushed a value bounded at 0.1 down to 0 when the derived step was integral, which the serializer rightly refuses — clamping now happens last. And `coFloatOrPercent` produced a bare number where the setting expected `50` or `50%`; its bounds do not say which interpretation they constrain, so it is refused rather than guessed, since a percent stepped as an absolute silently rewrites what the setting means. 191/191 unit, 5/5 integration, 80/80 project, 12/12 slice, 8/8 settings, and 1/1 XR files pass | Automated review; **this is the mechanism, not the surface** — no XR panel renders scoped settings yet, so P6.5's "identical draft/commit state across desktop, touch and XR" is still proven for two of three. The derived step is a judgement rather than a pinned upstream value: upstream's own spin controls may increment differently, and nothing here compares them |
| `EVID-121` | P6.5, P10.9, P2.6, P0.2 | Current worktree atop `f514992`; commit pending | `9fd12ff...` | 2026-08-18 | `npm --prefix web run quality`; `src/settings/editor/scopedStepper.test.ts`; `src/ui/xr/__tests__/xr-scoped-settings.test.ts`; `src/project/__tests__/scoped-settings-cross-surface.test.ts`; the keyboard-less step in `scripts/e2e-smoke.mjs` | Node 22.21.0; Chrome for Testing with the production CSP; headless; **no headset**, no hardware | Pass, and it closes the gap `EVID-040` opened. XR now has a scoped-settings surface: a target picker over the same five scopes, and one row per setting the scope supports. It is a view of the existing editor rather than a second one — the controller runs the **same query** the DOM panel runs (asserted against `SettingsDraftEditor.query` directly, field id by field id, in the same order), drives the same draft editor, and applies through the **same adapter object shape**, so a press is validated by the code that validates a typed character. Two structural corrections came out of building it. The project adapter was a single shared object whose stale guard is the snapshot it last handed out, so one surface's apply would silently move the other's guard — it is a factory now, one instance per surface, and a conflict is reported instead of hidden. And the XR UI adapter gained text and generic append, which is what let the drawing be tested at all: 9 traces click the rendered nodes and require each press to arrive with the field id and direction its row claims, that a refused setting gains no buttons but keeps its value, that an overridden row is distinguishable, and that a value is rewritten in place rather than by rebuilding forty spatial panels. 6 controller traces cover the query, refusals, the guard (a stale revision is refused, not applied to whatever is current), target cycling narrowing the rows — a part offers no `brim_type` — and one canonical apply per press. The cross-surface test no longer imitates the third surface: it drives the real controller, and the project a press produces is byte-identical to the project a typed field produces, including both generated 3MF configs. In the production browser the controller reports 46 rows at the project scope with 236 unsupported, and a press changes `sparse_infill_density` and undoes it as one entry. 191/191 unit, 5/5 integration, 80/80 project, 12/12 slice, 9/9 settings, and 2/2 XR files pass; bundle main 2,164,075 bytes, JS total 10,259,368 | Automated review; **no headset has rendered this**. Every claim here is Node or desktop Chrome, so P10.5's session is what would turn "the third surface exists" into "the third surface works" — reach, legibility, and whether a hundredth-of-range step is the right size for a controller are all questions only a session answers. The unsupported settings are summarised in one line rather than drawn, which is a stated divergence from the DOM panel; and the surface is reachable only from the XR Panels section, not from a spatial context menu (P11.2) |
| `EVID-122` | P6.5, P10.9, P0.2 | Current worktree atop `3dd075c`; commit pending | `9fd12ff...` | 2026-08-18 | `npm --prefix web run quality`; `src/settings/editor/scopedStepper.test.ts`; the keyboard-less step in `scripts/e2e-smoke.mjs` | Node 22.21.0; Chrome for Testing with the production CSP; headless; **no headset**, no hardware | Pass: the XR settings panel now follows the selection, which is the difference between a surface that is reachable and one that is usable — reaching an object by cycling past every plate, object, part and height range is not a way to choose a node with a controller. `scopedOverrideTargetIdForSelection` answers only for a selected *instance*, because nothing in the selection model names a part or a height range and guessing one from a click would edit a node nobody chose; those stay reachable by cycling. Only a changed selection moves the panel, and a cleared selection leaves it where it was. Building the browser proof found a real ordering defect in the proof itself rather than in the feature: the first version asserted the follow after a `select_all` that was already satisfied, so no selection change was published and the wait timed out. Deselecting first is what makes it a change — and the deselect is now its own assertion, pinning the stay-put behaviour. 46 project rows and 36 object rows in the production browser. 199/199 unit, 5/5 integration, 85/85 project, 12/12 slice, 10/10 settings, and 1/1 XR files pass; bundle main 2,164,659 bytes, JS total 10,260,115 | Automated review; **still no headset**. Whether a controller can comfortably reach a row 30 deep in a scrolled spatial list is exactly the question P10.5's session answers and this does not. The first gate run failed in `offline-smoke` — the known service-worker flake, not a regression — and passed on re-run through the same step |
| `EVID-123` | P11.2, P0.2, P10.9 | Current worktree atop `81d43e6`; commit pending | `9fd12ff...` | 2026-08-18 | `npm --prefix web run quality`; `src/ui/dom/__tests__/ContextMenu.test.ts`; `src/ui/dom/__tests__/ObjectsPanel.test.ts`; `src/actions/__tests__/registry.test.ts`; the right-click step in `scripts/e2e-smoke.mjs` | Node 22.21.0; Chrome for Testing with the production CSP; headless; **no headset**, no hardware | Pass: the context menu is a placement of the action catalog rather than a second catalog. An action names its targets, `surfacesFor` derives `dom-context` and `xr-context` from that under the same XR rule as every other surface, and one component renders it for the scene and the Objects tree alike — the tree's hand-built menu is gone, and its own two operations (Reveal, Rename, which act on the tree and exist in no catalog) now sit above the catalog's rather than instead of them. 14 object entries and 10 plate entries, disjoint by assertion, each reachable from at least one non-context surface so a person who never right-clicks loses nothing. 9 component traces cover what is easy to get subtly wrong twice: a disabled row cannot be chosen and states the registry's reason, a label is inserted as text and never as markup, Escape returns focus to where the click came from, arrows skip disabled rows and wrap, an outside press dismisses, an empty menu does not open, and a menu opened at the edge stays in the window. In the production browser, right-clicking the bed gives the plate menu, right-clicking a model selects it and gives the object menu, and choosing Mirror on X lands as one undoable canonical command. Building the browser proof produced a finding worth keeping: **a model's projected bounding-box centre is not necessarily a point that picks it** — ordinary for a shape that is not convex — so the workspace gained `getInstancePickPoint`, which searches the model's projected extent and returns a point **the picker itself confirms**. A left click at the centre selecting nothing looked at first like a picking defect; it is not, and the seam is what distinguishes the two. 200/200 unit, 5/5 integration, 85/85 project, 12/12 slice, 10/10 settings, and 1/1 XR files pass; bundle main 2,170,417 bytes, JS total 10,265,873 | Automated review; **the XR context menu is not a spatial popup**. It is a selection-scoped section at the head of the Panels menu, because no long-press channel on a model exists in the headset yet, and no headset has rendered it. The P0 surface-manifest classification pass — the other half of P11.2's acceptance — is untouched, and neither the outline nor the navigator has had a visual review |
| `EVID-124` | P11.2, P0.1, P0.2, P11.7 | Current worktree atop `1026924`; commit pending | `9fd12ff...` | 2026-08-18 | `npm --prefix web run quality`; `src/actions/__tests__/upstream-surfaces.test.ts`; `tools/parity/surface-map.json`; `docs/parity/snapmaker-v2.3.4.json`; `docs/parity/release-report.json` | Node 22.21.0; **no headset, no hardware** | Pass: P11.2's other acceptance clause — "the P0 surface manifest has no unclassified item" — is now a check rather than a claim. All 205 upstream menu leaves (162 distinct symbol/label items) are classified: **98 action, 7 panel, 17 container, 15 adaptation, 25 absent**. The five kinds are the point. A container is a submenu heading the extractor records beside real entries, and dropping it silently would shrink the denominator; a panel is reachable but not catalogued; and adaptation and absence are never summed, because one says the item will not arrive and the other says it has not arrived yet. Validation crosses three separately maintained files — the pinned manifest, the reviewed overlay, and the live registry — so drift in any one fails: an unclassified leaf, a stale entry classifying something upstream no longer has, an action id that is not in the catalog, or an `ADAPT-nn`/task id the plan does not define. Four negative traces prove each failure fires rather than being assumed. **Every one of the 98 mapped actions reaches a DOM surface and the command palette, and is either on an XR surface or states why not** — that sweep found no gap, which is worth recording as a negative result. The release report now carries the same counts plus the 25 absent items with the task that owes each. 201/201 unit, 5/5 integration, 85/85 project, 12/12 slice, 10/10 settings, and 1/1 XR files pass; bundle main 2,170,417 bytes | Automated review; **classification is not implementation**. 25 upstream items remain absent, and this row claims only that each is named, owed to a task, and explained — not built. The classification itself is a reviewed judgement: a reviewer who disagrees that, say, `Reload from disk` is `ADAPT-01` rather than a gap would be disagreeing with a line of JSON, which is the point of writing it down. The other manifest families (gizmos, preview modes and filters, format filters, device pages) keep their task dispositions and are **not** classified this way |
...
| `EVID-129` | P10.4.4, P10.4, P0.2 | Current worktree atop `fe91f92`; commit pending | `9fd12ff...` | 2026-08-19 | `npm --prefix web run quality`; `npm --prefix web run direction:check`; `npm --prefix web run test:pseudo` | Node 22.21.0; Chrome for Testing 150.0.7871.24; **no headset, no hardware** | Pass: the two layout defects that only appear in translation now fail the build instead of shipping. **The run found one on its first pass, which is the point of it:** the tool rail was a fixed 158 px measured against English, and a 40 % expansion truncated eight labels at once — the same truncation a German operator would have got from "Stützstrukturen bemalen" with nothing failing. The rail now sizes to `max-content` between a floor and a cap. A second defect came out of the language switch itself: the DOM shell appends to the primary bar rather than clearing it, because the hidden file input the Load action needs lives there, so remounting on a language change duplicated every button; the shell now tracks what it added. `direction:check` is static, because a rendered check only finds the physical properties that happen to be on screen in whatever state a smoke test reached — 54 declarations were converted to logical properties to make it pass, and two exceptions are allowed with reasons: `left: 50%` (fifty percent is the same distance from either edge, and converting the centring idiom would actively break RTL) and a `direction:physical` marker for a context menu that must open at the pointer's viewport coordinate in any writing direction. `test:pseudo` measures 152 critical controls across six groups for geometric overflow in both pseudo-locales, and **refuses to run against fewer than forty** — a green "none clipped" over an empty selector list is worse than no gate, because it is believed. Mirroring is asserted rather than assumed: the inspector must cross the viewport centre between the two directions, not merely have `dir="rtl"` set. Both gates were proven to bite by introducing a violation and watching them fail. 206/206 unit, 5/5 integration, 86/86 project, 13/13 slice, 10/10 settings, 3/3 l10n, and 1/1 XR files pass | Automated review; **this finds clipping, not crowding.** A geometric check cannot judge legibility, so P10.5's headset review is still where a human decides whether a mirrored, expanded layout is usable. None of the twenty upstream languages is RTL, so no *shipped* language exercises the mirrored path — the pseudo-locale is the honest substitute, not an equivalent, and no real right-to-left catalogue has ever been rendered. XR panel text is not measured at all, because the run drives the DOM shell; a clipped label in a headset panel would pass this gate |
| `EVID-130` | P11.1, P8.1, P8.3, P11.2, P0.2 | Current worktree atop `HEAD`; commit pending | `9fd12ff...` | 2026-08-22 | `npm --prefix web run quality`; `src/project/persistence/__tests__/recentProjects.test.ts`; `tools/parity/surface-map.json` | Node 22.21.0; Chrome for Testing 150.0.7871.24; **no headset, no hardware** | Pass: implements recent projects persistent bounded store (`RecentProjectsStore`), recording open/save project operations, tracking storage origins and metadata, and integrating the `file_open_recent` action into the registry and surface map. Adds the four remaining generic calibration action generators and bindings (`calib_flow_yolo_perfectionist`, `calib_junction_deviation`, `calib_input_shaping_frequency`, `calib_input_shaping_damping`) into `CalibrationRampGenerator`, `OrcaWorkspace`, and `ActionRegistry`, advancing surface-map action coverage to 115 mapped actions with absent menu leaves reduced to 8. All 210 unit and integration test files pass, l10n synchronization and sweep verified, bundle budgets, a11y, pseudo-localization, and E2E browser smokes verified. | Automated review; recent projects in web sandbox use local-storage/session fallback without OS file path handles; hardware verification of calibration runs pending P8.6 |
| `EVID-131` | P9.5, P9.6, P10.7, P0.2 | `3986517` (options) and `d55c950` (the sheet correction) | `9fd12ff...` | 2026-08-27 | `npm --prefix web run quality`; `src/printer/__tests__/print-start-options.test.ts`; `src/ui/xr/__tests__/xr-print-submission.test.ts`; the send step in `scripts/e2e-smoke.mjs`; Moonraker `/printer/objects/list`, `/printer/gcode/help`, `/server/info`, `/machine/timelapse/settings`, `/printer/gcode/script` | Node 22.21.0; Chrome for Testing with the production CSP; simulated Moonraker over real HTTP and WebSocket; **no hardware, no headset** | Pass. Sending from OrcaXR did less than sending from the desktop slicer — no plate calibration, no timelapse — and both are now offered, in the flat dialog and in the headset's sheet. Neither is declared by this app: 14 traces pin that an unqueried printer is *unknown* rather than incapable, that the command run is the one **that** printer named (`BED_MESH_CALIBRATE` where there is a `bed_mesh` section, `G29` where that is what `/printer/gcode/help` reports, nothing where neither is), that one endpoint failing leaves only its own capability unknown, that an option the printer never offered is refused before it reaches the machine, that nothing is ticked by default, and that the timelapse setting is written before the toolhead moves while the probe is awaited last with a 15-minute allowance. Nothing edits the G-code: the published artifact stays bound to the semantic snapshot it was sliced from, so levelling is its own command rather than an injected line. In the production browser both boxes are ticked and the printer's own command log shows `timelapse:true` and `gcode:BED_MESH_CALIBRATE` **before** `start`. The XR sheet was found fabricating while this was built: it drew an invented `Extruder T1 / PLA / #ff6d00` slot row whenever any tool summary existed, and literal `1h 15m` / `35 g` estimates. Those are gone — an unknown duration or weight now reads `not reported`, and a nozzle or bed nobody reported is left out of the header entirely. 226/226 unit, 5/5 integration, 90/90 project, 10/10 settings, 3/3 l10n, 11/11 XR, and 13/13 slice files pass; 28 axe rules pass; bundle main 2,326,320 bytes, JS total 10,534,035 | Automated review; **no printer levelled a plate here** — a simulator accepting `BED_MESH_CALIBRATE` proves the order and the refusals, not that a probe ran. Supervised hardware qualification of both options remains, as does a per-tool slot map for the XR sheet, which shows the send confirmation's own summary rather than a per-tool mapping |
| `EVID-132` | P9.6, P10.7, P0.2 | Current worktree atop `3986517`; commit pending | `9fd12ff...` | 2026-08-27 | `npm --prefix web run quality`; `src/printer/__tests__/printer-camera.test.ts`; the camera step in `scripts/e2e-smoke.mjs` (a second HTTP service on its own port) | Node 22.21.0; Chrome for Testing with the production CSP; simulated Moonraker plus a separate camera service, both over real HTTP; **no hardware** | Pass, and it corrects `EVID-044`. That entry recorded "an absolute URL on another host keeps only its path" as a safety property; it was also silently rewriting the *common* case. On a stock Klipper box Moonraker answers on 7125 and the camera on 8080, so `http://printer:8080/?action=snapshot` was reduced to `/?action=snapshot` and fetched from 7125 — a URL that answers nothing, which is why the panel sat on "Waiting for the first frame…" indefinitely. `resolveCameraSource` now decides against the endpoint the session is actually connected to, in three readings, and 11 traces pin them: Moonraker's own origin is a path through the credentialed transport; the same host on another port is fetched directly and **without** the API key, because that service never issued one; another host is refused with both origins named. The e2e drives a real second HTTP server and asserts the frame came from *that* port and that no `x-api-key`, `authorization`, or key-bearing query string ever reached it. Two more defects were found by driving the browser rather than the module. `view_webcam` only opened a `<details>` that lives on the Device page, so choosing Tools ▸ View Webcam from Prepare changed nothing an operator could see; it now shows the Device workspace first, and the e2e asserts the tab, the page, and the section. And the panel's polling watched the tab and the section but not the workspace, so a camera on a hidden page kept fetching; it now follows the page's `hidden` attribute. A frame that fails is reported where the picture would be — with a bounded 8-second wait, the URL named, and cross-origin refusal named as a cause — instead of an indefinite "waiting". 226/226 unit, 5/5 integration, 90/90 project, 10/10 settings, 3/3 l10n, 11/11 XR, and 13/13 slice files pass; 28 axe rules pass; bundle main 2,326,320 bytes, JS total 10,534,035 | Automated review; **no real camera was shown** — the cross-port service here allows cross-origin reads, and LNA does not bypass CORS, so a camera service configured to refuse them is still a diagnosis rather than a picture. Live MJPEG/WebRTC/HLS (`ADAPT-12`), an XR camera surface, and supervised hardware qualification all remain |
| `EVID-133` | P9.6, P10.7, P0.2 | Current worktree atop `98983d2`; commit pending | `9fd12ff...` | 2026-08-28 | `npm --prefix web run quality`; `src/printer/__tests__/printer-camera.test.ts`; the camera step in `scripts/e2e-smoke.mjs`; and — the part that made this findable — the printer itself: `curl http://192.168.1.228:7125/server/webcams/list`, then that URL asked of both ports | Node 22.21.0; Chrome for Testing with the production CSP; simulated Moonraker plus a separate camera service over real HTTP; **a real Snapmaker (192.168.1.228, camera-streamer behind nginx) was measured with `curl`, but no frame from it was rendered in a browser here** | Pass, and it finishes what `EVID-132` only half-corrected. That entry fixed absolute cross-port URLs and left the *common* form untouched: this printer reports `snapshot_url: "/webcam/snapshot.jpg"`, relative, and the panel still asked the API port for it. Measured rather than assumed — port 80 answers `200 image/jpeg` (195,830 bytes), port 7125 answers `404` — so a relative path now resolves to the printer's **web** origin, keeping the API path as a second candidate. The verbatim payload from that machine is pinned as a test. The second finding was that fetching the bytes could not have worked either: nginx returns the snapshot with **no `access-control-*` headers at all** (checked with an `Origin:` header), so a cross-origin read is blocked however it is addressed. An image is not, and needs no credential — so `cameraMechanisms` orders `image`, `direct`, `transport`, callers walk it, and the first that works is kept. The e2e now models a *secured* Moonraker whose snapshot endpoint refuses a request with no key: the browser tries to load the frame itself, is refused (`unauthenticatedSnapshots > 0`), falls back to the credentialed transport, and the caption then names the route actually feeding it. A second camera on its own HTTP service is displayed directly, and that service asserts no `x-api-key`, no `authorization`, and no key-bearing query string ever reached it. Building it surfaced a defect of its own, found with a focused probe rather than the suite: the image route completes synchronously and notifies, re-entering `applyPolling`, which claimed its timer *after* asking for the first frame — every re-entry started another interval, giving **1,575 requests in about a second** and a blown stack. The timer is claimed first now; the same probe reports a steady 4 fps. 226/226 unit, 5/5 integration, 90/90 project, 10/10 settings, 3/3 l10n, 11/11 XR, and 13/13 slice files pass; 28 axe rules pass | Automated review; **still no picture from real hardware in a real browser**. On the hosted HTTPS app this camera remains unshowable — an HTTP image is mixed content, and the byte route needs a cross-origin header nginx does not send — so the panel reports that dead end rather than showing a frame. Proving the fix end to end needs either an HTTP page on the LAN or a CORS header on the printer |
| `EVID-134` | P9.6, P10.7, P0.2 | Current worktree atop `6222a74`; commit pending | `9fd12ff...` | 2026-08-28 | `npm --prefix web run quality`; two puppeteer probes driving the built app against the **real printer** (one over HTTP, one over HTTPS with a self-signed certificate); `fix-webcam-cors.py` | Node 22.21.0; Chrome for Testing with the production CSP; **a real Snapmaker at 192.168.1.228 — camera-streamer behind nginx 1.24.0 — and a real 1920×1080 frame rendered in the panel from it** | Pass, and it is the first time any of this feature's claims has been checked against hardware rather than a simulator. Both routes were driven end to end against that machine. On an HTTP page the browser displays the camera itself: 31 frames from `http://192.168.1.228/webcam/snapshot.jpg`, `naturalWidth` 1920, and not one request to the API port. On an HTTPS page — the hosted app's own situation, where an HTTP image is mixed content — the same frames arrive as bytes, which needed the printer to allow the read: `fix-webcam-cors.py` adds that header to the four `/webcam*/` locations, named to `orcaxr.martinez.fyi` and localhost rather than `*`, because `*` would let any site the operator visits read a camera in their house. The script backs the file up, validates with `nginx -t`, rolls back if that fails, and is a no-op on a second run; the rollback path was exercised locally against a deliberately failing `nginx -t` before the real machine was touched. Verified after: the allowed origin gets `Access-Control-Allow-Origin` and `Vary: Origin`, an unrelated origin gets neither, and Fluidd, the camera, and the Moonraker API all still answer 200. The probes also caught a defect the suite could not: a single dropped frame retired the route that had been working, so the panel walked off a route serving 30 frames onto one that answers 404, and the caption then claimed those frames were "authenticated" when nothing had authenticated them. A route that has produced a frame is now proven and kept; only an unproven one is retired, and the image route proves itself through the `<img>` load event, which is the only thing on that side that knows. 226/226 unit, 5/5 integration, 90/90 project, 10/10 settings, 3/3 l10n, 11/11 XR, and 13/13 slice files pass; 28 axe rules pass | Automated review, with hardware in the loop for the first time. Still unproven: the Local Network Access grant itself — the HTTPS probe used `--allow-running-insecure-content` to stand in for a permission a headless browser cannot be asked for, so what is proven there is the CORS half. The `gui` camera on `/screen/` is left as it was, and a printer whose camera sits somewhere neither route reaches is still only as diagnosable as its own error message |
| `EVID-135` | P9.6, P10.7, P0.2 | Current worktree atop `fdf9bb9`; commit pending | `9fd12ff...` | 2026-08-28 | `npm --prefix web run quality`; a puppeteer probe against the **real printer** that screenshots the panel and reports both frame buffers | Node 22.21.0; Chrome for Testing with the production CSP; **the Snapmaker at 192.168.1.228, and its actual camera view** | Pass, and it is the entry that justifies screenshotting rather than asserting. `EVID-134` established that frames arrive — 31 of them, `naturalWidth` 1920 — and every one of those assertions was true while the panel showed **a uniform grey box**. Comparing the screenshot against the same frame fetched with `curl` (a purple bowl mid-print) is what exposed it. Two defects, both invisible to a 1×1-pixel fixture. An `<img>` goes blank the moment its `src` changes, so a single element pointed at the network shows the backing colour for as long as each frame takes to arrive; the panel now double-buffers, loading into a back buffer and swapping only once the frame has decoded. And assigning a new `src` *cancels* the load in flight, so polling faster than the camera can answer meant no frame ever finished at all: this camera needs longer than the 250 ms poll to produce a 195 KB 1920×1080 JPEG, and every tick was aborting the previous one. Only one load is now outstanding at a time, which lets the picture arrive at whatever rate the camera actually manages, with a 10-second cap so a stalled connection cannot wedge the panel. The probe now screenshots the panel, and the screenshot shows the print. 226/226 unit, 5/5 integration, 90/90 project, 10/10 settings, 3/3 l10n, 11/11 XR, and 13/13 slice files pass; 28 axe rules pass | Automated review, hardware in the loop. The suite still cannot catch this class on its own: every fixture here serves a 1×1 PNG that decodes instantly, so neither the blanking nor the cancellation reproduces. What would catch it is a fixture that answers slowly with a large frame, which is not written |
...
| `ADAPT-12` | A live MJPEG, WebRTC, or HLS camera feed | Snapshots at up to 4 fps, paused whenever nobody is watching: displayed straight from the camera when it is a separate service (no credential, and no CORS needed), and fetched as bytes through the credentialed transport when Moonraker itself serves them | The picture updates a few times a second instead of continuously, and a camera that offers only a stream is listed with that reason rather than shown | An `<img>` or `<video>` pointed at the printer cannot send `x-api-key`, so on a secured printer the feed would be a broken image; making it load would mean putting the key in the URL, which the transport refuses. Snapshots keep the credential in a header and work for every service type | Engineering | P9.6, P10.7, hardware camera qualification | Platform-adapted / proposed |
...
| 2026-08-18 | Answer every upstream menu item one at a time, in five kinds, and check the answer against the manifest, the overlay, and the live catalog | A task-level disposition says who owns a leaf, not whether an operator can reach it, so 205 upstream items were "dispositioned" and unanswered. The five kinds exist because collapsing them would hide the distinction that matters: an adaptation says the item will not arrive and an absence says it has not arrived yet, and a single "missing" bucket lets the gap list shrink by relabelling. Checking across three separately maintained files is what stops the classification from becoming a stale copy of a catalog it no longer describes | P11.2, P0.1, P0.2, P11.7 | 162 items classified (98 action, 7 panel, 17 container, 15 adaptation, 25 absent), 6 traces including four that prove each failure mode fires; the 25 absent items remain unbuilt |
| 2026-08-18 | Make a context menu a *placement* of the action catalog — an action names its targets, the registry derives the surfaces, one component renders them | Two menus answering the same right-click is the reachability gap P11.2 exists to close, and the repository already had one: a hand-built two-entry menu in the Objects tree and nothing at all in the scene. Deriving the surface from the catalog means a new action cannot be reachable by right-click in one shell and missing in another, and an entry cannot disagree with the menu bar about whether it is available. Actions that cannot run are deliberately excluded rather than shown disabled — they stay reachable where their reason is stated, and a shortcut surface full of rows that only explain themselves is not a shortcut | P11.2, P0.2 | 9 component traces, 1 registry trace, 1 panel trace, and a production-browser right-click that mirrors a model as one undoable command; the P0 manifest pass and a headset session pending |
| 2026-08-18 | Give XR its scoped settings as a *view* of the existing editor — same query, same draft, same adapter — and draw it through the mockable UI adapter | A second settings implementation is how two surfaces start disagreeing about what a project holds, so the only thing this surface owns is how a value is named: pressed rather than typed. Building it turned up the shared project adapter, whose stale guard one surface could move under the other; it is a factory now. Drawing through the adapter matters for a different reason — the recurring XR failure is a control that renders and changes nothing, which until now only a real browser could catch, and a headset cannot be a prerequisite for noticing it | P6.5, P10.9, P2.6 | 15 headless traces plus a production-browser press that changes a setting and undoes it; a Galaxy XR session (P10.5) pending |
...
| 2026-08-27 | Offer a pre-print option only when the printer's own answer supports it, and run the command that printer named | The desktop slicer offers plate calibration and a timelapse at send time and OrcaXR offered neither, but neither is something an app can declare: a probe and a timelapse component are things a machine has or has not. `/printer/objects/list`, `/printer/gcode/help`, and `/server/info` are asked, and "nobody has asked yet" is kept distinct from "cannot" — the failure mode worth avoiding is a panel confidently reporting a capability nobody looked for. Levelling runs as its own awaited command rather than an injected G-code line, so the published artifact stays bound to the snapshot it was sliced from | P9.5, P9.6 | 14 option traces plus a production-browser send whose command log shows both arriving before `start` (`EVID-131`) |
| 2026-08-27 | Decide where a reported camera URL points against the endpoint in hand — Moonraker's path, the same host on another port, or somewhere else — instead of keeping the path and discarding the origin | Discarding the origin read as a safety property and was also the bug: it re-pointed the ordinary crowsnest camera at Moonraker's port, where it answers nothing, which is what left the panel waiting for a first frame forever. A camera on another port is a different service, so it is fetched directly and without the printer's key — it never issued one — while a different *host* stays refused, because a camera list is printer-host content and following it off-host would make the page a request forwarder | P9.6, P10.7 | 11 camera traces plus an e2e second HTTP service that receives the frame request and no credential (`EVID-132`) |
| 2026-08-28 | Ask the printer where its camera actually is before writing anything, and let the browser display a camera that takes no credential | Two assumptions had survived because nobody measured: that a relative snapshot path belongs to the API endpoint, and that a frame can always be fetched as bytes. One `curl` at the machine disproved both — the path is served on port 80 and 404s on 7125, and the response carries no `access-control-*` header at all, so a cross-origin read is blocked however it is addressed. An `<img>` needs no CORS and no credential, which makes it the route that works; the credentialed fetch stays for the case it exists for, a camera Moonraker serves itself. Ordering the routes and keeping the first that works means no one has to be asked which arrangement their printer has | P9.6, P10.7 | The printer's own answers, 14 camera traces including its verbatim payload, and an e2e that models a secured Moonraker so the fallback happens for the reason it exists (`EVID-133`) |
| 2026-08-28 | Keep a camera route that has produced a frame, and let the printer say which pages may read it | Driving the real machine showed both halves. A route walked off after one dropped frame — cameras drop frames — onto a route that could never work, and the caption then described the frames on screen by a route that had not fetched them; proving a route by an actual frame and never retiring a proven one is what makes the walk converge instead of wander. And on the hosted HTTPS app the byte route is the only one left, so the camera has to allow the read: a header named to specific origins, not `*`, because a camera in somebody's house should not be readable by every site they visit | P9.6, P10.7 | Two probes against the printer at 192.168.1.228 — 31 frames displayed directly on an HTTP page, the same frames read as bytes on an HTTPS one — plus the header verified present for the named origin and absent for others (`EVID-134`) |
| 2026-08-28 | Look at the panel, not only at its assertions, when hardware is finally in the loop | Every assertion about the camera passed while the panel showed a grey box: frames arrived, `naturalWidth` was 1920, the caption was right, the route was right. What was wrong was the one thing nothing asserted — whether the picture was on screen. An `<img>` blanks when its `src` changes and cancels the load in flight when it changes again, and a 1×1 test fixture decodes too fast for either to show. Double-buffering and one-load-at-a-time are the fixes; comparing a screenshot with `curl` output is what found them | P9.6, P10.7 | A panel screenshot showing the print, against the same frame fetched directly (`EVID-135`) |
| YYYY-MM-DD |  |  |  |  |
