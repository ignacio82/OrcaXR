...
- [~] **P11.2 — Close every menu, toolbar, context, camera, and view gap.** File, Edit, View, Add,
  ...
  - **The classification pass (`EVID-124`):** the manifest's 205 upstream menu leaves were all
    ...
    across when `EVID-125` built them, which is the gap list working as one.) Adaptation and absence are kept apart deliberately — one says the item
  ...
  - **Missing:** neither the outline nor the navigator has had a visual review, and the 22 absent
    ...
    printability, scale-to-build-volume, and per-object STL export. Classification is not
