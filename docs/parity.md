...
- [~] **P11.2 — Close every menu, toolbar, context, camera, and view gap.** File, Edit, View, Add,
  ...
  - **Missing:** the G-code window has no surface yet — the model is built and traced, the panel is
    not. The remaining context-menu surfaces and the P0 surface-manifest classification pass are
    untouched. Neither the outline nor the navigator has
    had a visual review.
