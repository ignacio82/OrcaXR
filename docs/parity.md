...
- [~] **P11.2 — Close every menu, toolbar, context, camera, and view gap.** File, Edit, View, Add,
  ...
  - **The classification pass (`EVID-124`):** the manifest's 205 upstream menu leaves were all
    ...
    across when `EVID-125` built them, and scale-to-build-volume when `EVID-126` did — the gap list
    working as one: **102 reached, 21 absent** as of this row.) Adaptation and absence are kept apart deliberately — one says the item
  ...
  - **Missing:** neither the outline nor the navigator has had a visual review, and the 21 absent
    items are absent — the largest are the SLA menus (out of scope for FFF targets), the
    auxiliary-file browser, input-shaping and junction-deviation calibrations, per-object
    printability, per-object STL export, and instance-to-object promotion. Classification is not
