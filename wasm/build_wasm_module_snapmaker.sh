#!/usr/bin/env bash
# OrcaXR: link the Snapmaker-Orca-based WASM slicer module (slic3r.mjs +
# slic3r.wasm) — libslic3r built from the Snapmaker/OrcaSlicer fork so the
# web engine matches the external slicer container's CLI bit-for-bit at the
# source level (FullSpectrum mixed-filament resolution included). Run after
#   ninja -C "$SNAPMAKER_TREE/build-wasm" libslic3r
# Pass the fork checkout via SNAPMAKER_TREE (default: third_party/SnapmakerOrca).
set -euo pipefail

HERE="$(cd "$(dirname "$0")" && pwd)"
ORCA="${SNAPMAKER_TREE:-$HERE/../third_party/SnapmakerOrca}"
BIN="$ORCA/build-wasm"
# Cross-compiled deps from the fork's own superbuild (deps/build-wasm) —
# same emsdk + recipes as the upstream port (Boost 1.84, CGAL, TBB 2021.13).
...
ARCHIVES=(
    ...
    # (inflate*/deflate*) and a second definition collides at link.
)
