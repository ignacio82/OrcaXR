...
// Apply flat key=value JSON overrides (a flattened OrcaSlicer profile from
// the web ProfileLoader). Rejections are logged, never fatal.
void apply_overrides_json(Slic3r::DynamicPrintConfig &cfg, const std::string &overrides_json)
{
    if (overrides_json.empty()) return;
                      ...
                      const char *out_path, std::string *statistics_json = nullptr)
{
    ...
    fprintf(stderr, "[orcaxr] print.apply done (cfg fil_dia=%zu fil_col=%zu | print fil_dia=%zu fil_col=%zu)\n",
            cfg.option<Slic3r::ConfigOptionFloats>("filament_diameter", true)->values.size(),
            cfg.option<Slic3r::ConfigOptionStrings>("filament_colour", true)->values.size(),
            print.config().filament_diameter.values.size(),
            print.config().filament_colour.values.size());
                         ...
                         int num_filaments = 1)
{
    ...
    apply_overrides_json(cfg, overrides_json);
    ...
    // Normalize the per-filament vectors AFTER the overrides so a short or
    // malformed override (e.g. a comma-joined filament_colour, which
    // ConfigOptionStrings parses as ONE value — ';' is the separator) can't
    // leave them under-sized. Every filament-count consumer downstream
    // (PrintApply num_extruders = filament_diameter.size(), ToolOrdering's
    // filament_map indexing, GCodeProcessor's per-filament caches) reads
    // these sizes; a mismatch silently dropped volumes past filament 1 and
    // OOB-crashed multi-material tool ordering on wasm32.
    if (num_filaments > 1) {
        cfg.set_num_filaments((unsigned int)num_filaments);
        // filament_map only exists on engines with the BBS multi-extruder
        // machinery (upstream Orca 2.3.x); the Snapmaker fork's older base
        // has no such option — create=true would die on the missing def.
        if (cfg.def() != nullptr && cfg.def()->has("filament_map")) {
            auto &fmap = cfg.option<Slic3r::ConfigOptionInts>("filament_map", true)->values;
            if (fmap.size() != (size_t)num_filaments)
                fmap.assign((size_t)num_filaments, 1); // all filaments on extruder 1
        }
    }
    fprintf(stderr, "[orcaxr] config ready\n");
    return run_print(model, cfg, out_path);
}
...
/**
 * Slice a PROJECT 3MF the way the desktop/CLI slicer does: the model AND
 * the embedded project config (Metadata/project_settings.config) are both
 * loaded via load_bbs_3mf, so the effective config matches a
 * `orca-slicer --slice` run on the same file — including FullSpectrum
 * mixed-filament definitions and per-part extruder assignments that the
 * flat-STL paths cannot express. [overrides_json] is applied on top of
                                ...
                                std::string *statistics_json = nullptr)
{
    ...
    write_all(in_path, bytes);
    // Early stage print: the 3MF parse below can run tens of seconds on a
    // big project and emits nothing itself — the web client's stall
    // watchdog needs a sign of life before it.
    fprintf(stderr, "[orcaxr] parsing project 3mf (%zu bytes)...\n", bytes.size());
    ...
    // LoadModel extracts mesh objects out of the BBS plate containers,
    // LoadConfig pulls the embedded project DynamicPrintConfig. Do NOT add
    // LoadAuxiliary — the importer would extract project resources to disk
    // paths that don't exist in MEMFS (same trap as the Android JNI).
    ...
    const bool ok = Slic3r::load_bbs_3mf(in_path, &loaded, &subs, &model,
                                         &plate_data, &project_presets,
                                         &is_bbl, &file_version,
                                         /*proFn=*/nullptr, strategy,
                                         /*project=*/nullptr, /*plate_id=*/0);
    for (auto *p : plate_data) delete p;
    std::remove(in_path);
    if (!ok || model.objects.empty())
        throw std::runtime_error("project 3MF load failed or contains no objects");
    // Per-plate custom gcodes (color change / pause markers) ride along in
    // model.plates_custom_gcodes; GCode reads them via
    // get_curr_plate_custom_gcodes(), so no copy is needed here.
    fprintf(stderr, "[orcaxr] project loaded: %zu object(s), is_bbl=%d, %zu config keys\n",
            model.objects.size(), (int)is_bbl, loaded.keys().size());
    {
        for (const auto *obj : model.objects) {
            const auto *oe = obj->config.option("extruder");
            fprintf(stderr, "[orcaxr-dbg] object '%s' extruder=%s volumes=%zu\n",
                    obj->name.c_str(), oe ? oe->serialize().c_str() : "-",
                    obj->volumes.size());
            for (const auto *vol : obj->volumes) {
                const auto *ve = vol->config.option("extruder");
                fprintf(stderr, "[orcaxr-dbg]   vol '%s' extruder=%s\n",
                        vol->name.c_str(), ve ? ve->serialize().c_str() : "-");
            }
        }
        for (const char *k : {"mixed_filament_definitions", "wall_filament",
                              "sparse_infill_filament", "solid_infill_filament",
                              "mixed_filament_advanced_dithering",
                              "dithering_step_painted_zones_only",
                              "filament_colour", "wall_loops", "wipe_tower_filament"}) {
            const auto *o = loaded.option(k);
            fprintf(stderr, "[orcaxr-dbg] cfg %s = %s\n", k,
                    o ? o->serialize().c_str() : "(unset)");
        }
    }

    Slic3r::DynamicPrintConfig cfg = Slic3r::DynamicPrintConfig::full_print_config();
    fixup_enum_keys_map(cfg);
    cfg.apply(loaded, /*ignore_nonexistent=*/true);
    apply_overrides_json(cfg, overrides_json);
    // The CLI normalizes before Print::apply (OrcaSlicer.cpp
...
/// Synchronous project slice on the CALLING thread (no pthread). Used by
/// SAFE_HEAP / sanitizer diagnostic runs in Node, where instrumented memory
/// checks deadlock if the slice runs on a worker pthread. Not used by the web
/// app (which needs the async start/poll API to keep the event loop free).
std::string slice_project_sync(const std::string &path, int max_threads,
                               const std::string &overrides_json)
{
    ...
    try {
        return slice_project_inner(read_all(path.c_str()), overrides_json);
    } catch (const std::exception &e) {
        return std::string("ORCAXR_ERROR: ") + e.what();
    } catch (...) {
        return std::string("ORCAXR_ERROR: unknown C++ exception");
                         ...
                         const std::string &overrides_json)
{
    ...
    std::thread([path, max_threads, overrides_json]() {
        ...
        tbb::global_control tbb_limit(
            tbb::global_control::max_allowed_parallelism, (size_t)threads);
        try {
            g_slice_result = slice_project_inner(read_all(path.c_str()), overrides_json);
        } catch (const std::exception &e) {
            g_slice_result = std::string("ORCAXR_ERROR: ") + e.what();
        } catch (...) {
