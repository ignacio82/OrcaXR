...
const projectPath = process.argv[2] ?? join(process.env.HOME, 'Downloads/PeggyPalette.3mf');
...
const t0 = Date.now();
mod.startSliceProject('/tmp/orcaxr_proj.3mf', Number(process.env.ORCAXR_TBB ?? 4), '{}');

const gcode = await new Promise((resolve, reject) => {
  const timer = setInterval(() => {
    const out = mod.pollSlice();
    if (out.length > 0) { clearInterval(timer); resolve(out); }
    else if (Date.now() - t0 > 1_200_000) { clearInterval(timer); reject(new Error('timed out')); }
  }, 200);
});
...
console.log(`project slice OK in ${((Date.now() - t0) / 1000).toFixed(1)} s: ${(gcode.length / 1048576).toFixed(1)} MB, ${layers} layers, ${toolChanges} tool changes, tools ${tools.join(',')}`);
if (tools.length < 2) {
