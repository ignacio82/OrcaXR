...
mod.startSlicePainted(
  ...
  2, Number(process.env.ORCAXR_TBB ?? 4), JSON.stringify(overrides),
);
...
console.log(`painted slice OK in ${Date.now() - t0} ms: ${(gcode.length / 1024).toFixed(0)} KB, ${layers} layers, ${toolChanges} tool changes`);
if (toolChanges < 1) {
