...
### A2. FullSpectrum engine emission 🟢 Shipped — **painted Local-Z + 3MF round-trip + unpainted LayerCycle all device-verified GREEN**; Pointillisme remains ⚪ deferred (`#if 0`'d upstream FS v0.9.9)

> **Patch stack (shipped):** the FS v0.9.9 Local-Z emission port — formerly the unwritten 0027–0034 / 0044–0067 series — landed and was **consolidated into `patches/0053-fullspectrum-local-z-consolidated.patch`** (2026-05-13, commit `20d666b`) plus the resolver/round-trip/region patches 0015–0052 + 0069–0071. `scripts/build_native.sh` applies the full chain 0001–0071 CLEAN from a reset v2.3.2 submodule (the old "halts at 0053" note is dead). Source-of-truth pinned at FS tag `v0.9.9` (`ratdoux/OrcaSlicer-FullSpectrum`, commit b3c41fda) — read **semantically**, not via `git apply` (the fork sits on Snapmaker/OrcaSlicer 2.3).

Mixed-filament data layer + Kotlin plumbing (`MixedFilamentEntry` v1 fields, v0.9.9 wire format, `SAFE_KEYS`/`PROJECT_OVERRIDE_KEYS` whitelist, per-row Advanced editor, `set/delete_mixed_filament_row` MCP) all at FS v0.9.9 parity.

**Verified GREEN on device (SM-I610 Galaxy XR arm64-v8a, 2026-05-17, clean `libslic3r.a` from a full from-patches build, `tests=3 failures=0`):**
- `FullSpectrumLocalZTest` — the FS sublayer planner emits the LZ5g phase-b marker + sub-base Z-steps only in painted layers. **Local-Z emission works end-to-end.**
- `FullSpectrumRoundtripTest` — v0.9.9 `mixed_filament_definitions` round-trips byte-identically through 3MF (patch 0034).
- `PeggyPaletteFullSpectrumParityTest` — 38-part painted-Local-Z parity vs desktop FS, gated on per-tool MODEL extrusion ±2 % (re-gated by `1b4ef8b`; the old "+5.4 % total-mm" was a purge-minimisation **win**, resolved & stale — do **not** bisect `afc6f7d..e8a7cad`).

**Unpainted LayerCycle — SHIPPED via patches `0072` + `0073`.** Empirical instrumented re-test (2026-05-17) disproved the earlier "Gap 3 dominant / it's just config propagation" framing *for the test path*: the bundled `FullSpectrumLayerCycleTest` was blocked **solely by Gap 1** — the `Slic3r::Model::add_object` `extruder=1` stamp clobbered the project-level virtual `wall_filament` in `region_config_from_model_volume`, so `MixedFilamentManager::resolve` never ran. Fix shipped as two numbered patches:
- **`patches/0072-fullspectrum-region-config-virtual-propagation.patch`** — when the parent/project config targets a virtual row (`wall_filament > num_physical`) and the object stamp knocked it to a physical id, restore the parent virtual id (FS-desktop semantic). PeggyPalette unaffected (its project wall is physical, so the guard never fires).
- **`patches/0073-fullspectrum-reorder-cadence-preservation.patch`** — `ToolOrdering::reorder_extruders_for_minimum_flush_volume`'s `get_custom_seq` pins every layer to its resolved `lt.extruders` cadence when `mixed_filament_definitions` is non-empty, so the stock flush-minimiser DP no longer collapses the per-layer alternation. (Pure-config patches 0072/0073 here are the *current shipped* ones — distinct from the same-numbered earlier prototypes that were reverted; the empirical re-test established Gap 1 propagation is the real fix, not the disproven emission-pipeline port.)

Pointillisme stays deferred — `#if 0`'d in FS v0.9.9 upstream.

**Verified GREEN on device (clean from-patches `build_native.sh` applying 0001–0073, Galaxy XR arm64-v8a):** `FullSpectrumLayerCycleTest` (un-`@Ignore`'d, real unpainted `wall_filament=<virtual>` UX, hardened ≥40-of-each + ≥80%-per-layer-alternation assertions) **alternates T0/T1 per layer**; `FullSpectrumLocalZTest` + `FullSpectrumRoundtripTest` + `PeggyPaletteFullSpectrumParityTest` all stay GREEN (0072/0073 don't perturb the painted-Local-Z path). Full mechanism + probe trail: `docs/proposals/fullspectrum-layercycle-engine.md`.
