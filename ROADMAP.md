...
### A2. FullSpectrum engine emission 🟡 Partial — **painted Local-Z + 3MF round-trip SHIPPED & device-verified GREEN**; only unpainted **LayerCycle** remains (the Gap-3 emission-pipeline port); Pointillisme is `#if 0`'d upstream
...
**Still missing — unpainted LayerCycle only.** The resolver chain (patches 0015/0017/0018/0019) is wired, but the resolved per-layer cadence does **not survive** `reorder_filaments_for_minimum_flush_volume` (`ToolOrderUtils.cpp`, zero MixedFilament awareness) + `_make_wipe_tower` into the emission `ToolOrdering`. This is **Gap 3** — the ~800-LoC `GCode.cpp` + ~311-LoC `ToolOrdering` + reorder-DP FS port. The "it's just config propagation" thesis is **disproven** (candidate patches 0072/0073 built, device-tested, reverted — no standalone benefit). Full mechanism, the proven-partial `get_custom_seq` fix, the open step (feed the *true* FS Local-Z cadence, not `lt.extruders`), and the proven build/iterate loop are in **`docs/proposals/fullspectrum-layercycle-engine.md` §"Real remaining work"** (mirrored in GEMINI.md gotcha #20). Pointillisme stays deferred — `#if 0`'d in FS v0.9.9 upstream.

**Entry criteria (LayerCycle):** semantic port against FS `v0.9.9`; ASAN sweep of the new emission sites vs the patch-0014 TBB serial shim; every iterate keeps `PeggyPaletteFullSpectrumParityTest` GREEN (shared reorder DP — never commit on red).

**Exit criteria:** a 2-physical + 1-virtual unpainted LayerCycle project emits alternating T0/T1 per layer; `FullSpectrumLayerCycleTest` is un-`@Ignore`'d (with the stricter ≥40-of-each + odd/even-layer assertions from the proposal's Phase 3) and passes on a connected arm64-v8a device; `FullSpectrumLocalZTest` + `FullSpectrumRoundtripTest` + `PeggyPaletteFullSpectrumParityTest` stay GREEN.
