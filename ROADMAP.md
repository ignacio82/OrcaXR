...
### C6. MCP Server for OrcaXR (AI Control & Smart Assistant) 🟡 Phase 1 + 2.x shipped — 57 tools live; per-volume editing + emboss + libslic3r abort hook pending
...
- Per-model per-volume editing (Phase XR_OBJ_4 multi-volume API) isn't yet exposed — adding modifier / negative / support volumes to an existing model.
- Embossing (D4 — `runEmboss`) isn't yet exposed; needs a richer parameter surface (font, depth, mode=Add|Subtract, optional SVG path).
- Painting via API: stamps require a 3D click point, which an LLM doesn't have. Smart-fill from a triangle index would be the right MCP shape; not yet plumbed.

**Verification:** instrumented test from a desktop machine — `curl -X POST http://<headset-ip>:7080/mcp -H "Authorization: Bearer <key>" -d '{"jsonrpc":"2.0","id":1,"method":"tools/list"}'` returns the 57-tool catalog. End-to-end smoke test for the "natural-language slicer" loop: `load_model_from_path` → `slice_active_plate` → poll `get_workspace_state` until `slice_state.kind == "done"` → `save_gcode_to_downloads`, then `select_printer` + `start_print` against a Moonraker host. Workspace tools require the app foreground (the `attached` flag in `get_workspace_state` is the gate).
