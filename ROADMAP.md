...
### A10. Variable / adaptive layer height per object 🟢 Shipped
...
**Shipped pieces:** native compute + Kotlin wrapper + per-object slice plumbing for both single-slice and multi-slice paths + 4 MCP tools + 11 unit tests pinning the param contract. **`AdaptiveLayerPanel` SpatialPanel** wired into the top-nav (Layers icon) — Quality slider (0..1, 0.05 increments), Smoothing radius slider (0..10), Keep-min checkbox, Compute / Clear-profile buttons routing through `WorkspaceAction.ComputeAdaptiveLayerHeights` / `SetLayerHeightProfile`. Per-Z bar chart shows the current profile's Z bands and per-band layer heights when one is set; capped to the last 40 bands so a 200-pair profile doesn't dominate the panel. Compute is disabled when nothing is selected; Clear is disabled when no profile is stored.
...
### A13. Wipe-tower auto-positioning 🟢 Shipped
...
### B13. Settings backup / restore as JSON 🟢 Shipped
...
### B12. Calibration print library 🟢 Shipped
...
**Shipped variable-ramp generator:** `CalibrationRampGenerator.generate(Params)` is pure Kotlin — given (kind, start, end, step, layersPerBand, layerHeightMm, firstBandZmm) it returns a `List<SlicerEngine.CustomGcodeTick>` with `kind = Template` and per-band M-command bodies (`SET_PRESSURE_ADVANCE` + `M572` for PA, `M104` for temperature, `SET_RETRACTION` for retraction length, `M221` for volumetric flow, `M220` for speed). Direction is inferred from `sign(end - start)` so the user can't break it with a wrong-sign step. Non-divisible step lengths emit a final clamped band at exactly `end`. New MCP tool `generate_calibration_ramp(kind, start, end, step, layers_per_band?, layer_height_mm?, first_band_z_mm?, plate_id?, mode?)` composes the generator with the A11 `add_custom_gcode_tick` action loop — `mode='replace'` (default) clears the plate's existing ticks before adding so a leftover tick from a prior calibration can't corrupt the new ramp. `CalibrationRampGeneratorTest` (8 unit tests covering PA / temperature / cooling tower direction / step-direction inference / band-height scaling / per-Kind body coverage / non-divisible step clamp / param validation). End-to-end LLM flow is now a one-liner: `add_handy_model("calib_pa_tower")` → `generate_calibration_ramp(kind="pressure_advance", start=0.02, end=0.06, step=0.005, …)` → `slice_active_plate`. Apply-to-profile path stays a manual user step (per the comment in the original outline).
...
### E8. Foreground-service background slicing 🟢 Shipped
...
**Shipped:** `SliceForegroundService` (mirrors `McpForegroundService` shape — same `dataSync` foregroundServiceType, same notification + open-app intent pattern). `SliceLifecycle` process-singleton with re-entrant counter so nested inner slices don't tear down the service prematurely; `activeLabel` / `progressPercent` / `progressMessage` StateFlows. Manifest declares the second foreground service + the cancel BroadcastReceiver. `runSlice` and `runSliceMulti` wrap their `scope.launch` body in `SliceLifecycle.beginSlice(ctx, label)` … `try { … } finally { SliceLifecycle.endSlice(ctx) }` so every return path (success / exception / coroutine cancel) releases the hold. The slicer status callback at the JNI bridge calls `SliceLifecycle.updateProgress(percent, message)` so the foreground notification now shows "OrcaXR slicing dragon — 42%" with a determinate progress bar AND the libslic3r phase message ("Generating support material") in the body line. **`nativeAbort` JNI hook landed**: a global `g_active_print` pointer (mutex-guarded, restored on scope exit so nested slices behave) lets `nativeAbort()` call `Slic3r::Print::cancel()` on the active slice; the next libslic3r cancellation poll throws `CanceledException`, the JNI catches at top level and returns rc=-5, Kotlin maps to new `SliceResult.Cancelled`. C6's `cancel_slice` MCP tool is no longer a no-op — routes through `SlicerEngine.abort()`. The notification gets a Cancel action that fires `SliceCancelReceiver` (manifest-registered so it works with the Activity torn down). `SliceLifecycleTest` (5 unit tests covering happy-path, nested begin/end, over-decrement clamping, progress flow publishing + clamping, and updateProgress no-op when inactive). On-device verification that a 30 s slice survives Activity teardown is the remaining instrumented-test follow-up.

### E9. Native build toolchain pin (NDK 26+ / Clang 17+) 🟢 Shipped
...
**Shipped:** GEMINI.md gotcha §29 documents the NDK ≥ 26 / Clang ≥ 17 pin with the u1-slicer B62 cross-reference and the `llvm-readelf -p .comment` verification command. `scripts/build_native.sh` now asserts `clang --version` is ≥ 17 before invoking CMake (fails fast with an explicit "u1-slicer B62" error message). `verifyNdkClangFloor` Gradle task scans every `libslic3r_jni.so` under `app/build/intermediates` for a `clang version N` marker and fails the build if N < 17; wired as `finalizedBy` of `mergeDebug/ReleaseNativeLibs` so it runs as part of every `assembleDebug` / `assembleRelease`. Pin is enforced at four layers now (gradle `ndkVersion` + bash assert + GEMINI gotcha + `verifyNdkClangFloor`) so removing any one fails loudly.
