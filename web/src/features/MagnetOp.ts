...
export enum MagnetShape { DISC = "DISC", BLOCK = "BLOCK" }
...
export class MagnetOp {
    ...
    static primitiveFor(spec: MagnetSpec, cavityHeightMm: number): [PrimitiveKind, number[]] {
        if (spec.shape === MagnetShape.DISC) {
            return [PrimitiveKind.CYLINDER, [
                ((spec.diameterMm || 0) + 2 * (spec.clearanceMm ?? 0.2)) / 2,
                cavityHeightMm,
                2,
            ]];
        } else {
    ...
    static placeCavities(
        ...
        if (ch > maxCh) {
            warnings.push(`Model too short: magnet pocket (${chRaw.toFixed(2)} mm) + 2×roof (${(2*roof).toFixed(2)} mm) exceeds model height (${hostHeightMm.toFixed(2)} mm); pocket clamped.`);
