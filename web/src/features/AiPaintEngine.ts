...
export enum SlabAxis {
  X,
  Y,
  Z,
}
export enum SlabTest {
  Centroid,
  AnyVertex,
  AllVertices,
}
export enum ConeSign {
  Outward,
  Inward,
  Both,
}
...
export class AiPaintEngine {
  ...
  public static normalCone(
    ...
    for (let i = 0; i < bvh.triCount; i++) {
      ...
      let match = false;
      switch (sign) {
        case ConeSign.Outward:
          match = dot >= cosThreshold;
          break;
        case ConeSign.Inward:
          match = dot <= -cosThreshold;
          break;
        case ConeSign.Both:
          match = Math.abs(dot) >= cosThreshold;
          break;
      }
      if (match) out.push(i);
    }
  ...
  public static surfaceRegion(
    bvh: any,
    seedTri: number,
    maxDihedralDeg: number,
    maxTriangles: number = 65536,
  ): Int32Array {
    return bvh.smartFillBfs(seedTri, maxDihedralDeg, maxTriangles);
  }

  public static geodesicDisc(
    bvh: any,
    seedTri: number,
    radiusMm: number,
    maxDihedralDeg: number = 60,
    maxTriangles: number = 65536,
  ): Int32Array {
  ...
  private static vertexAxisInRange(
    bvh: any,
    tri: number,
    axis: SlabAxis,
    lo: number,
