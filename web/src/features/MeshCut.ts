...
export function cutByPlane(positions: ArrayLike<number>, nx: number, ny: number, nz: number, d: number): CutResult {
  ...
  const push = (arr: number[], a: V3, b: V3, c: V3) => {
    arr.push(...a, ...b, ...c);
  };
  ...
  for (let t = 0; t < triCount; t++) {
    ...
    if (nNeg === 0) {
      push(pos, v[0], v[1], v[2]);
      continue;
    }
    if (nPos === 0) {
      push(neg, v[0], v[1], v[2]);
      continue;
    }
    ...
    const lone = nPos === 1 ? s.findIndex((x) => x > EPS) : s.findIndex((x) => x < -EPS);
    const a = v[lone],
      b = v[(lone + 1) % 3],
      c = v[(lone + 2) % 3];
    const sa = s[lone],
