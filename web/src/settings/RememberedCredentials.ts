...
export interface RememberedCredentials {
  /** Moonraker API key for the configured printer. */
...
export function saveRememberedCredentials(
  ...
  try {
    if (!credentials.remember) {
      storage.setItem(STORAGE_KEY, JSON.stringify({ printerApiKey: '', slicerToken: '', remember: false }));
      return;
    }
    if (!printerApiKey && !slicerToken) {
      storage.removeItem(STORAGE_KEY);
      return;
    }
    storage.setItem(STORAGE_KEY, JSON.stringify({ printerApiKey, slicerToken, remember: true }));
  } catch {
