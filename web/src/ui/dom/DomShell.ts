...
export class DomShell {
  ...
  mount(hosts: Hosts): void {
    ...
    // Grouped header menus
    const menu = this.registry.byDisclosure('menu');
    this.buildMenu(hosts.addMenu, `${domIcon('library')} Add`, menu.filter((a) => a.menuSection === 'add'));
    this.buildMenu(hosts.toolsMenu, `${domIcon('advanced')} Tools`, menu.filter((a) => a.menuSection === 'tools'));
  ...
  private buildMenu(host: HTMLElement, triggerLabel: string, actions: Action[]): void {
    ...
    for (const a of actions) {
      ...
      item.title = a.hint ?? a.label;
      item.innerHTML = `<span class="glyph">${domIcon(a.icon)}</span><span>${a.label}</span>`;
      item.onclick = () => {
