import { SettingsConfig, type SettingSchema, type SettingGroup, type SettingSection, type SettingRow } from '../../actions/SettingsConfig';
...
export class SettingsInspector {
  ...
  private render() {
    ...
    tabsContainer.style.cssText = 'display:flex; gap:2px; background:var(--oxr-color-bg-sunken, #0000004d); padding:4px; border-radius:var(--oxr-radius-sm, 8px); margin-bottom:12px;';
    ...
    groupsContainer.style.cssText = 'display:flex; gap:6px; overflow-x:auto; padding-bottom:8px; margin-bottom:4px; scrollbar-width:none;';
    ...
    settingsArea.style.cssText = 'flex: 1; overflow-y: auto; padding-right: 4px; display: flex; flex-direction: column; gap: 16px;';
  ...
  private buildSection(section: SettingSection): HTMLElement {
    ...
    title.style.cssText = 'font-size: 11px; font-weight: 700; color: var(--oxr-color-text-muted, #a0aab5); text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 8px; border-bottom: 1px solid var(--oxr-color-stroke, #ffffff1a); padding-bottom: 4px;';
  ...
  private buildRow(row: SettingRow): HTMLElement {
    ...
    rowEl.style.cssText = 'display: flex; justify-content: space-between; align-items: center; font-size: 13px; color: var(--oxr-color-text, #fff);';
    ...
    if (row.type === 'b') {
      ...
      chk.style.cssText = 'width: 16px; height: 16px; accent-color: var(--oxr-color-accent, #ffb74d); cursor: pointer; margin: 0;';
      chk.onchange = () => {
      ...
      input.style.cssText = 'width: 60px; box-sizing: border-box; background: var(--oxr-color-bg-sunken, #0000004d); color: #fff; border: 1px solid var(--oxr-color-stroke, #ffffff1a); border-radius: 6px; padding: 4px 6px; font-size: 13px; text-align: right; outline: none;';
      input.oninput = () => {
        let val = input.value;
      ...
      select.style.cssText = 'background: var(--oxr-color-bg-sunken, #0000004d); color: #fff; border: 1px solid var(--oxr-color-stroke, #ffffff1a); border-radius: 6px; padding: 4px 6px; font-size: 12px; outline: none; cursor: pointer;';
